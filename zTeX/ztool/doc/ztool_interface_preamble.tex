    ...
    \clist_map_inline:nn {#2}
      {
        \clist_put_right:Nn \l__ztool_doc_source_clist 
          {
            \subsection{##1}
            \texhighfile[first-line-number=1]{../code/#1/ztool.#1.##1.tex}
          }
      }
  }
...
\newcommand{\ztexDocPrintSource}{%
  \ztexslideTF{}{
    \clearpage
    \section{zTool 源码}
    本节列出 \pkg{ztool} 宏包的源码, 供有需要的用户参考. 另, 本节所有打印源码的对应行号已经过特殊处理，用户可直接从 PDF 中复制源码样例，加以修改使用.
    ...
    \subsection{ztool.sty}
    \texhighfile[first-line-number=1]{../../ztool/code/ztool.sty}
    \clearpage
