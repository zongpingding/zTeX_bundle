...
\begin{function}[added=2024-04-27]{\test}
  \begin{syntax}
    \cs{test}\oarg{arg1}\meta{\upshape{AAA\string|BBB}}
  \end{syntax}
...
\begin{document}
\ExplSyntaxOn
\tl_set:Ne \l_tmpa_tl {*-YYY}
\tl_replace_all:Nnn \l_tmpa_tl {*}{XXX}

\tl_set:Ne \l_tmpb_tl {\exp_not:N \l_tmpa_tl}
% \tl_show:N \l_tmpb_tl
% \tl_use:N \l_tmpa_tl
\ExplSyntaxOff
...
\lipsum[1][1-5]
\begin{align}
  \sum_{i=1}^\infty \frac{1}{i^2} = \int_a^b x\:\mathrm{d}x.
\end{align}
...
{\Arial Hello world,\itshape Hello world.}
...
% ==> ztex normal commands test
\InputIfFileExists{zlatex-cfg.tex}{}{}
% \documentclass{article}
% \usepackage[T1]{fontenc}
% \usepackage{garamondx}
\documentclass[class=l3doc]{../code/ztex}
% \ztexset{mathSpec={font=ptmx}}
\zthmstyle{leftbar}
% \zfontset{math=euler} % works
% \zfontset{text=times} % works 
% \zfontset{doc=lmm} % works
% \zthmhook*[theorem]{before=, begin=YYY}
...
constantly equal to 1 on $N\cap\operatorname{supp}_M(\omega)$. Since $f\omega$ coincides with $\omega$ on $N$, 
both integrals are unchanged when $\omega$ is replaced by $f(\omega)$, so we may assume that $\omega$
has compact support.

\begin{theorem}[Local Chart]
  Choose a smooth atlas on $M$ consisting of charts of the type of Definition 10-5
  ...
  \begin{align*}
    ...
    &&
    \int_N \:\mathrm{d}\omega = \sum_{\alpha} \int_N \:\mathrm{d}(\rho_\alpha\omega)
  \end{align*}
  and a subordinate smooth partition of unity $(\rho\alpha)_{\alpha\in A}$. The formulas
\end{theorem}
...
% \begin{center}
%   \mbox{}\vfill
%   \includegraphics[width=.9\paperheight, angle=90]{ztex_options.pdf}
%   \vfill\mbox{}
% \end{center}
% \restoregeometry
\end{document}
...
% ==> zlatex commands rename
% 1. Search Setup:
% include:*.tex, *.sty, *.cls
% exclude:zlatex_manual.tex, .git/*, build/*

% 2. Rename Pattern:
% Setup --> setup (all of them are lowercaes)
% zlatex + thm + <*>   --> \zthm<*>
% zlatex + page + <*>  --> \zpage<*>
% set/setup --> set ###
% create 	--> new
...
% Navigate --> nav
% Internal commands:
% \__zlatex_color_set:n --> \__ztex_color_setup:n (\__ztex_<module>_setup:)

% 3. TODO
% add scatterr align command:use the 's' option in \makebox
% implement \appmatter and \backmatter
\InputIfFileExists{zlatex-cfg.tex}{}{}
\documentclass[class=l3doc]{../code/zlatex}
...
\ztexloadlib{alias}
\ztexset{
  mathSpec={font=ptmx, envStyle=fancy},
}
% \zslideset{
...
\zslidethemeuse[
  ...
  BL={bg=white, text=},
  BC={bg=white, text=},
  BR={bg=white, text=\zslideframeind/\zslideframeall{\Roman{section}}},  
]{AnnArborSpruce}
\ExplSyntaxOn
...
\AddToHook{shipout/foreground}{
  \zslide_status_info:nnnn {head}{0}{1}{
    \zslidenavsym[
      ...
      \RPITEM
      % \textcolor{gray}{\rule{1em}{1em}}
    ]
  }
}
\def\SecRation{\zslideframeind/\zslideframeall{\Roman{section}}}
\newcommand\ITEM[1][gray]{\hspace*{0pt}\pgfornamenthan[color=#1, scale=0.06]{39}}
\newcommand\RPITEM[1][gray]{\prg_replicate:nn {\int_eval:n {36*\SecRation}}{\ITEM[#1]}}
\ExplSyntaxOff
...
reduce the problem to the case where $\omega\in\Omega^n_c(M)$. $\supp_M(\omega)\subseteq U$
...
\begin{align*}
  \omega = \sum_{i=1}^{n }{f_i(x)\dd x_1\wedge\ldots\wedge\widehat{\dd x_i}\wedge\ldots\wedge\dd x_n},
\end{align*}
and choose $b>0$ such that $\supp_{\RR^n}f_i \subseteq [-b, b]^n, 1\le i\le n$. Using Theorem 3-12,
\begin{align*}
  \omega_{|\partial\RR^n_-}
  = f_1(0, x_2, \ldots, x_n)\dd x_2\wedge\ldots\wedge\dd x_n.
\end{align*}
...
\begin{align}\label{eq:10-3}
  ...
  = \int f_1(0, x_2, \ldots, x_n)\dd\mu_{n-1}.
\end{align}
By Theorem 3-7 we have 
\begin{align*}
  \dd\omega 
  = \sum_{i=1}^{n}{(-1)^{i-1}\frac{\partial f_i}{\partial x_i}\dd x_1\wedge\ldots\wedge\dd x_n}.
\end{align*}
Hence 
...
\begin{align*}
  & \int_{-\infty}^\infty \frac{\partial f_i}{\partial x_i}(x_1, \ldots, x_{i-1}, t, x_{i+1}, \ldots, x_n)\dd t \\
  =\;\; & f_i(x_1, \ldots, x_{i-1}, b, x_{i+1}, \ldots, x_n) 
    - f_i(x_1, \ldots, x_{i-1}, -b, x_{i+1}, \ldots, x_n).\\
  =\;\; & 0
\end{align*}
...
When $i=1$, one gets 
\begin{align*}
  \int_{-\infty}^0 \frac{\partial f_1}{\partial x_1}(t, x_2, \ldots, x_n)\dd t
  & = f_1(0, x_2, \ldots, x_n) - f_1(-b, x_2, \ldots, x_n)\\
  & = f_1(0, x_2, \ldots, x_n),
\end{align*}
and by Fubini's theorem 
\begin{align}\label{eq:10-6}
  \int_{\RR^n_-} \frac{\partial f_1}{\partial x_1}\dd\mu_n
    = f_1(0, x_2, \ldots, x_n)\dd\mu_{n-1}.
\end{align}
By combining Equations (6), (10) the desired formula follows.
\zlatexaliasOff
\end{document}
...
\DeclareMathSymbol{\pi}{0}{CMMletters}{"19}



\begin{document}
Let $1< p\le 2$. A Banach space $X$ is said to have \textbf{Haar type} $p$ if there exists
a constant $c\ge 1$ such that

\begin{align}
  \bigg(\int_0^1 \Big\|\sum_{k=0}^n \mathbf{d}_k(t)\Big\|^p\bigg)^{1/p} 
  \le 
  2\pi\alpha\cdot\gamma\bigg(\prod_{i=1}^\infty\sum_{k=0}^n \int_0^1 \|d_k(t)\|^p\bigg)^{1/p} 
\end{align}
for every $X$-valued Walsh–Paley martingale of arbitrary length $n$.

Let $X_\mathbb{R}$ be a real linear lattice. Then an absolute value is obtained on the complex-
ification $X_\mathbb{C} = X_\mathbb{R}+iX_\mathbb{R}$ by letting
\begin{align}
  |x+iy| := \sup_{0\le s\le 2\pi} |x\cos s + y\sin s|.
\end{align}


%%%%%%% Use \box_show:N to see the font details:
\ExplSyntaxOn
% \hbox_set:Nn \l_tmpa_box {\textit{\bfseries d}}
\hbox_set:Nn \l_tmpa_box {$\pi$} %\pi=\OML/cmm/m/it/10
% \box_show:N \l_tmpa_box
% > \box13=
% \hbox(7.01+0.14)x5.17
% .\OT1/ntxtlf/b/it/10 d
% .\kern 0.17
\ExplSyntaxOff
\end{document}
...
% ==> make alias command only valid in a group.
\InputIfFileExists{zlatex-cfg.tex}{}{}
\documentclass[]{../code/zlatex}
% \usepackage{ascii}
\ztexloadlib{alias}
\zlatexaliasOpSet{rot=Rot}
\makeatletter
% \let\asciiFF\FF
% \let\FF\z@FF
% \newcommand\internal@cmda[1]{CMD-A:#1}
% \def\aliasOn{\let\cmdA\internal@cmda}
% \def\aliasOff{\def\cmdA{UNDEFINED}}
...
\ExplSyntaxOn
\clist_map_inline:Nn \g__zlatex_alias_user_clist {\string #1\par}
\vskip3em
\hrule

\clist_map_inline:Nn \g__zlatex_alias_internal_clist {\string #1\par}
\ExplSyntaxOff
...
\makeatletter
\zthmtitleformat*{\color{\thm@tmp@color}\bfseries【\zthmname\;\zthmnumber】\ }
\zthmtitleformat*[proof]{\color{\thmproof@tmp@color}\kaishu\underline{\zthmname}.\;\,}
\makeatother
\zthmcnt{share=true}
...
\zthmcolorset{proof=blue!50, solution=green!50}
% \zthmtitlebefore{THM}
% \zthmtitlebefore[proof]{PRF}
% \zthmtitlebefore[proof]{|PRF-LIKE|}
% \zthmbefore{}
% \zthmbefore[proof]{\par}
\makeatletter
\zthmtitleformat*{\bfseries\color{\thm@tmp@color}\zthmname|\zthmnote{\{}{\}}|\zthmnumber\ }
\zthmtitleformat*[proof]{\color{\thmproof@tmp@color}\bfseries|:\scshape\zthmname:|\ }
\makeatother
...
This is an Zproof zthmnew-1.
\end{Zproof}

\begin{Zexample}[zthmnew-2]
This is an Zexample zthmnew-2.
\end{Zexample}
...
\zthmtitleformat{\bfseries\color{red}\zthmnumber|\ :|\zthmnote{[}{]}:(\zthmname)}
...
% ==> bug of \zthmtoc and \ztexptoc bug in 'slide' mode
\documentclass{article}
...
%%%%%% zlatex config %%%%%%%
\zthmstyle{lapsis} % must bofore the loading of 'theme' library
\ztexloadlib{alias, theme}
\zthmcolorset{remark=red!75, definition=orange!75}
\newcounter{PRB}
\zthmnew{proof}{problem={PRB\ \thePRB}|red}
\zpagemask[anchor=tl, position={(0pt, \zph)}]{\parbox{\paperwidth}{\color{gray}\itshape\huge\lipsum[1-4]}}
% \zthmtocsym{
% definition=\ding{74}\;,
% remark=\ding{98}\;,
% }
...
\zthmlang{fr}
\zthmtitleformat*{\color{green}\zthmnumber|\ :|\zthmnote{[}{]}:(\zthmname)}
%%%% slide setup
\zslideset[doc]{
bg-color=yellow!20,
text-color=red
...
\zthmstyle{lapsis}
\ztexloadlib{alias, theme}
\zthmcolorset{remark=orange!75, definition=green!75}
\newcounter{PRB}
\zthmnew{proof}{problem={PRB\ \thePRB}|red}
\usepackage{lipsum}
\usepackage{zhlipsum}
...
\ztexloadlib{alias}
% \zlatexaliasOpSet{alt=ALT, im=IM}

\zthmstyle{background}
\zthmnew{theorem}{Ztheorem=NEW-THM|orange}
% \zthmcolorset{theorem=blue!50} % EXPETC: works ---> pass
\zthmcolorset{Ztheorem=blue!50}  % EXPETC: works ---> pass
% \zthmcolorset{link=blue!50}    % EXPETC: error ---> pass
\begin{document}
...
\newpage
\zthmtocadd[section]{name=NEW-SEC}
\zthmtocstop
\begin{proposition}[PROP]
...
\zslidethemeuse[
  UR={text=\zLaTeX{} Slide:\ \zslidenavsym},
]{AnnArborSpruce}
...
\AddToHook{shipout/background}{
  \zslide_status_bar:nnnn {BC}{(.5\paperwidth, -.5\paperheight)}{.2}{80pt}
}
\AddToHook{shipout/foreground}{
  \zslide_status_info:nnnn {foot}{.66}{.75}{AAA\zslidenavsym[\textcolor{red}{XX}][\textcolor{gray}{XX}]}
}
\ExplSyntaxOff
% END TEST INTERFACE
...
% \zpagemask{\transparent{.1}\includegraphics{Epmmy.jpg}}
% Hello world
...
\zthmstylenew{
  aaa={begin=, end=, option=\zthmtitleswitch},
}
% \zthmtitleswitch
\zthmstyle{aaa}
...
\usepackage{lipsum}
% \zthmstyle{lapsis}
% \ztexset{mathSpec={envStyle=shadow}}
\ztexloadlib{theme}
\zthmnew{theorem}{Ztheorem=NEW-THM|{HTML}{8e22ad}, Zproposition} 
% \zthmtitleformat{\bfseries\zthmname\ \zthmnumber\zthmnote{(}{)}}
% \zthmbefore{\quad}
...
% \zthmlang{fr}
\zthmcnt{share, parent=section}
\zthmtocprefix{\ding{118}\;}
\zcolorset{
  remark=red!50,
  axiom = gray!40,
...
\zpagemask[
  % label=FIRST, % EXPECT: Class zlatex Warning: Only star version of \zpagemask is label-allowed.
  anchor=bl,
  position={(0pt, -14em+\zph)}
]{%
...
\newpage
\zpagemask*[anchor=tc,label=TEST,layer=foreground]{%
...
\zthmtocadd[name=NEW-SEC]{section}
...
\makeatother
\zpagemask*[layer=foreground]{AAA}
\zpagemask[layer=foreground]{BBB}
...
\zslidepageTF{<2}{PAGE-II}{NOT PAGE-II}
...
% --> zlatex interface
% \ztextitleNew{page}{part}



% \ztextitleStyle{section}
% {
%   shape=display,
%   % align=left,
%   label=X-\thesection-Y.,
...
\zthmcnt{share, parent=subsection}
...
\section{TEXT}
\zpagemask{AAA}
\zpagemask{\includegraphics{Epmmy.jpg}}
\zpagemask[layer=foreground, position={(10em, 10em)}]{\parbox{3em}{BBB-1\\BBB-2}}
\zpagemask*[position={(.25\zpw, .25\zph)}]{\ifnum\thepage=2 CCC2\else CCC1\fi}
\zpagemask*[position={(0pt, 0pt)}, anchor=lb]{XXX-\thepage}
\zpagemask[position={(\zpw, \zph)}, anchor=tr]{YYY}
...
\newpage
\zthmtoc[vspace=15pt, title=THM LIST]
\end{document}
...
\end{definition}
\zthmstylenew{
  aaa={begin=\par\begin{center}, end=\end{center}, option=\zthmtitleswitch},
...
\zthmtitleformat{\bfseries\zthmnumber\ \zthmname}
% \ShowHook{zlatex/thm/titleformat}
...
\zthmhook{before=\color{blue}\begin{framed}, after=\end{framed}}
...
\ztexset{mathSpec={envStyle=fancy}}
\zthmnew{theorem}{Ztheorem=THM|{HTML}{8e44ad}, Zproposition} 
% \zslidethemeuse[
%   UR={text=\zslidenavsym},
% ]{AnnArborBeaver}
\NewMarkClass{test}
...
\zthmtitleformat{\textcolor{red}{\thmname{#1}:\thmnote{[#3]-}\thmnumber{#2}}}
...
\newpage
\zthmstyle{fancy}
\zthmhook{begin=BEGIN, end=END, after=AFTER}
% \zthmhook{begin=\begin{center}, end=\end{center}}
% \zthmhook{before=\begin{framed}, after=\end{framed}}
\begin{proposition}
  \boomen
\end{proposition}


\zthmstylenew{before=\color{blue}\begin{framed}, after=\end{framed}}
\begin{definition}
  \boomen
\end{definition}
...
% }
% \newsavebox{\zlatexBox}
% \zthmstylenew*{begin=\color{red}\begin{framed}, end=\end{framed}}
...
% \zcolorset{theorem=teal}
\zthmcnt{share, parent=section}
\zthmnew{theorem}{Zaxiom, Ztheorem=Thm|green, Zproposition=Prop|orange} %Ztheorem=Thm|{HTML}{0f45f3}
\zthmnew{proof}{Zproof, Zexample=Example|red, Zsolution=Solution|}
...
\zthmtitleformat{\thmname{#1}:\thmnote{[#3]-}\thmnumber{#2}}
\begin{lemma}[prime number]
  \boomen
\end{lemma}
...
\begin{document}
\newtheorem{aaa}{AAA}
\zthmtitleformat{\ThmName. \ThmNumber [\ThmNote)}
\begin{aaa}[SECOND]
  ENV-CONTENT-SECOND
\end{aaa}
\end{document}
...
% \zthmtitleformat{\thmname{#1}: \thmnumber{#2} \thmnote{[#3]}}
\begin{aaa}[FIRST]
  ENV-CONTENT-FIRST
\end{aaa}
...
\ztexset{mathSpec={envStyle=fancy, font=mathpazo}}
% \zslideset{
%   world=haha,
%   toc={hello=hi}
% }
