...
\begin{definition}[映射连续性]
  如果对于某一给定的点 $x_0\in X$, 映射 $T$满足下面的条件:
  对于任意给定的 $\varepsilon > 0$, 存在 $\delta > 0$, 使得当 $\rho(x, x_0) < \delta$时, 有 
  $\rho_1(Tx, Tx_0) < \varepsilon$,则称 $T$在点 $x_0$处连续. 如果 $T$在 $X$上的每一点都连续,
  则称 $T$在 $X$上连续, 且称 $T$为连续映射.
\end{definition}
...
\ExplSyntaxOff
\AddToHook{zlatex/thmstyle/before}{\par}
...
\zlatexThmSecAdd[name=NEW-SEC]{section}
...
\ExplSyntaxOn
\ztool_resize_to_ht:nn {2.5pt}{XXX}
\ExplSyntaxOff
...
\ExplSyntaxOn
m\ztool_resize_to_wd:nn {1em}{mmmm}\par 
...
\end{document}\begin{axiom}[AXIOM]
\boomen
\end{axiom}
