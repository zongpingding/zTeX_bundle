\ProvidesExplFile{ztex.module.thm.tex}
  {2026/02/12}{\ztex@versi@n}
  {thm~module~for~ztex}
...
% ==> module init
\cs_new_protected:Npn \zthmremoveCJKecglue 
  { \zfont_cancel_CJKecglue: }
\cs_new_protected:Npn \zthmrestoreCJKecglue 
...
\ztex_thm_create:nn {theorem}
  { 
    axiom, definition, theorem, lemma, 
...
\ztex_msg_set:nn {thm-name}
  {
    An~unexpected~math~env~name~in~multichoice~key:
    '\l_keys_key_str',~there~is~no~internal~config~for~it.
  }
...
\protected\def\zthmtitle{\@ifstar\@zthmtitle\@@zthmtitle}
...
\ztex_msg_set:nn {thm-color-set}
  {
    Your~color~spec~key~'#1'~is~not~in~the~
    thm~env~list,~please~check~it~again.
  }
\NewDocumentCommand{\zthmcolorset}{m}
  {
    \ztex_label_hook_preamble_last:nn 
      { ztex-thmcolor-setup-user }
      {
        \keyval_parse:nnn
          { \use_none:n }
...
\DeclareHookRule{env/document/before}
  {ztex-thmcolor-setup-user}
  {>}
  {ztex-thmenv-setup-user}
\@onlypreamble\zthmcolorset


% create new thm env(init related variables)
\ztex_keys_define:nn { thmnew }
  {
    name    .tl_set:N = \l__zthm_new_name_tl,
    color   .tl_set:N = \l__zthm_new_color_tl,
    tocsym  .tl_set:N = \l__zthm_new_tocsym_tl,
  }
\cs_new:Nn \__zthm_new_keyval_clear:
  {
    \tl_clear:N \l__zthm_new_name_tl
    \tl_clear:N \l__zthm_new_color_tl
    \tl_clear:N \l__zthm_new_tocsym_tl
  }
\cs_new:Npn \__ztex_thm_color_set:nn #1#2
  {
    \tl_if_empty:eTF { #2 }
      { \ztex_keys_set:nn {color}{#1=black} }
      { \ztex_keys_set:nn {color}{#1=#2}    }
  }
...
\cs_new:Npn \__zthm_envnew_var_init:Nnn #1#2#3
  {
    \ztex_thm_create:nn {#2}{#3}
    \__ztex_color_keyval_add:n {#3}
    \prop_gput:Nnn #1 { #3 }{ #3 }
    \prop_gput:Nnn \g_ztex_thm_toc_symbols_prop
      { #3 }{ #3 }
  }
\cs_new:Npn \__zthm_envnew_var_init:Nnnn #1#2#3#4
  {
    \ztex_thm_create:nn {#2}{#3}
    \__ztex_color_keyval_add:n {#3}
    \ztex_keys_set:nn { thmnew }{ #4 }
    \exp_args:Nno \__ztex_thm_color_set:nn
      { #3 }{ \l__zthm_new_color_tl }
    \tl_if_empty:NTF \l__zthm_new_name_tl
      {
        \prop_gput:Nnn #1
          { #3 }{ #3 }
      }{
        \prop_gput:Nno #1
          { #3 }{ \l__zthm_new_name_tl }
      }
    \tl_if_empty:NTF \l__zthm_new_tocsym_tl
      {
        \prop_gput:Nnn \g_ztex_thm_toc_symbols_prop
          { #3 }{ #3 }
      }{
        \prop_gput:Nno \g_ztex_thm_toc_symbols_prop
          { #3 }{ \l__zthm_new_tocsym_tl }
      }
    \__zthm_new_keyval_clear:
  }
\cs_new_protected:Npn \zthm_envnew_var_init:Nnn #1#2#3
  {
    \keyval_parse:nnn
      { \__zthm_envnew_var_init:Nnn  #1{#2} }
      { \__zthm_envnew_var_init:Nnnn #1{#2} }
      { #3 }
  }
\NewDocumentCommand{\zthmenvnew}{O{theorem}m}
  {
    \ztex_label_hook_preamble_last:nn 
      { ztex-thmenv-setup-user }
      {
        \zthm_envnew_var_init:Nnn
          \g__ztex_thm_name_prop
          { #1 }{ #2 }
      }
  }
\@onlypreamble\zthmenvnew
...
\ztex_msg_set:nn {mathEnv-dependency}
  {
    MathEnv~style:'\g__ztex_thm_style_tl'~requires~
    package~'tcolorbox'~and~'tikz',~and~either~of~
    which~hasn't~been~loaded~in~your~preamble.~
    Reset~to~default~'plain'~style~now.
  }
...
\ztex_msg_set:nn {mathEnv-style}
  {
    You~use~an~incorrect~MathEnv~style:~'\g__ztex_thm_style_tl',~
    All~of~the~pre-defined~styles~are:'plain',~'leftbar',~
    'background',~'fancy';And~styles~defined~inside~'thm'~
    library(load~it~by~command~'\token_to_str:N\ztexloadlib{thm}')~are:~
    'shadow',~'paris',~'lapsis',~'tcb',~'obsidian',~and~'elegant'.
  }
% thm counter
...
% thm env warper
\cs_new:Npn \__ztex_thm_wrap_start:nnn #1#2#3 
  {
    \def\thm@tmp@name{#1}
    \def\thm@tmp@color{\tl_use:c {l__ztex_#1_color_tl}}
    \__ztex_thm_theorem_title_gen:nnn {#1}{#2}{#3}
    \tl_if_exist:cTF
      { g__ztex_thm_style_\g__ztex_thm_style_tl _option_tl }
      { \tl_use:c {g__ztex_thm_style_\g__ztex_thm_style_tl _option_tl} }
      { \ztex_msg_error:n {mathEnv-style} }
    \tl_if_exist:cTF
      { g__ztex_thm_style_\g__ztex_thm_style_tl _begin_tl }
...
\newcommand\zthmtocprefix[1]
  {
    \tl_set:Nn \l__ztex_thm_toc_prefix_tl
      { \exp_not:n {#1} }
  }
\@onlypreamble\zthmtocprefix
\cs_new:Npn \__ztex_thm_wrap_end:n #1 
  {
    \tl_if_exist:cTF 
      { g__ztex_thm_style_\g__ztex_thm_style_tl _end_tl }
      { \tl_use:c {g__ztex_thm_style_\g__ztex_thm_style_tl _end_tl} }
      { \ztex_msg_error:n {mathEnv-style} }
    \zsect_add_theorem_line:eeoe
      ...
      {
        {
          ...
          \exp_not:n 
            { 
              \prop_item:Nn \g_ztex_thm_toc_symbols_prop
                { #1 }
            }
        }
...
\cs_new:Npn \__ztex_thm_theorem_title_gen:nnn #1#2#3 
    ...
    \bool_if:NTF \g__ztex_thm_cntshare_bool
      {
        \cs_set:Npn \zthmnumber 
          {
            \cs:w the\g__ztex_thm_cntparent_tl
              \cs_end:.\arabic{ztex@thm@sharecnt}
          }
...
\cs_new:Npn \__ztex_thm_proof_title_gen:nn #1#2 
  {% #1:env-name; #2:separator
    \cs_set:Npn \zthmname 
      {
        {\prop_item:cn {g__ztex_thm_name_prop}{#1}}
      }
    \def\thmproof@tmp@color
      { \tl_use:c {l__ztex_#1_color_tl} }
...
\NewDocumentCommand{\zthmtitlebefore}{O{theorem}m}
  {
    \tl_gset:cn {g__ztex_thm_#1_title_before_tl} 
      { #2 }
  }
...
% ==> Thm tocline interface
% NOTE: thm tocline relies on 'sect' module
\NewDocumentCommand\zthmtocstop{}
  {
    \ztoc_stop_table:n { lom }
  }
\cs_if_exist:NF \zsect_add_theorem_line:nnnn
  {
    \cs_set_protected:Nn \zsect_add_theorem_line:nnnn
      { }
  }
...
\NewDocumentCommand{\zthmtocadd}{O{section}m}
  {
    ...
    \edef\zthmtoc@tmp@target
      {
        zthm@toc-add.
          \int_use:N \g_zthm_added_toc_target_int
      }
    \MakeLinkTarget*{\zthmtoc@tmp@target}
    \group_begin:
    \ztex_keys_set:nn {thm/add}{#2}
    \zsect_add_theorem_line:nnoe {#1}
      {
...
\NewDocumentCommand{\zthmtoc}{O{}}
  {
    \group_begin:
    \exp_after:wN \exp_args:Nc \exp_after:wN \ztocformat
      \exp_after:wN { \g__ztex_thm_toc_level_tl }{ #1 }
...
\NewDocumentCommand{\zthmtocsym}{m}
  {
    \ztex_label_hook_preamble_last:nn 
      { ztex-thmtocsym-setup-user }
      {
        \prop_gput_from_keyval:Nn 
          \g_ztex_thm_toc_symbols_prop 
          { #1 }
      }
  }
...
\@onlypreamble\zthmtocsym
\@onlypreamble\zthmtocsymrm


% ==> thm env definition
% theorem-like env
\cs_new_protected:Npn \zthm_theorem_env_setup:n #1
  {% #1:env name; #2:env title seperator
    \zcnt_safe_new:oo { #1 }
      { \g__ztex_thm_cntparent_tl }
    ...
    \DeclareDocumentEnvironment{#1}{O{}}
      {
        \UseHook{ztex/thm-theorem/before}
        \UseHook{ztex/thm-theorem-#1/before}
        \__ztex_thm_wrap_start:nnn {#1}{##1}{\ }
        \MakeLinkTarget*{zthm@#1.\zthmnumber}
        \bool_if:NT \g__ztex_thm_title_inline_bool 
          {
            \group_begin:
              \zthm@title@before{theorem}
              \__ztex_thm_theorem_title:
            \group_end:
          }
        \UseHook{ztex/thm-theorem/begin}
        \UseHook{ztex/thm-theorem-#1/begin}
        \tl_trim_spaces:n
      }{
        \UseHook{ztex/thm-theorem/end}
        \UseHook{ztex/thm-theorem-#1/end}
        \__ztex_thm_wrap_end:n {#1}
        \UseHook{ztex/thm-theorem/after}
        \UseHook{ztex/thm-theorem-#1/after}
      }
  }
\ztex_label_hook_preamble_last:nn 
  { ztex-thmptheorem-setup-inner }
  {
    ...
    \clist_map_inline:Nn \g__ztex_thm_theorem_clist 
      {
        \zthm_theorem_env_setup:n {#1}
      }
  }

% proof-like env
\newcommand{\qedsymbol}{\ensuremath{\square}}
\cs_new_protected:Npn \zthm_proof_env_setup:n #1
  {% #1:env name; #2:env title seperator
    \DeclareDocumentEnvironment{#1}{O{}}
      {
        \UseHook{ztex/thm-proof/before}
        \UseHook{ztex/thm-proof-#1/before}
        \__ztex_thm_proof_title_gen:nn {#1}{\,} 
        \group_begin:
          \zthm@title@before{proof}
          \__ztex_thm_proof_title:
        \group_end:
        \UseHook{ztex/thm-proof/begin}
        \UseHook{ztex/thm-proof-#1/begin}
        \tl_set:Nn \l__thm_proof_name_tl {#1} 
        \tl_trim_spaces:n
      }{
        \UseHook{ztex/thm-proof/end}
        \UseHook{ztex/thm-proof-#1/end}
        \str_if_eq:VnTF \l__thm_proof_name_tl 
          { proof }
          { \hfill\qedsymbol\par }
          { \par }
        \UseHook{ztex/thm-proof/after}
        \UseHook{ztex/thm-proof-#1/after}
      }
  }
\ztex_label_hook_preamble_last:nn
  { ztex-thmprooof-setup-inner }
  {
    \clist_map_inline:Nn \g__ztex_thm_proof_clist 
      {
        \zthm_proof_env_setup:n { #1 }
      }
  }

% create thm env manually
\cs_new:Nn \__zthm_env_setup:nn
  {
    \exp_args:Nno \use:c
      { zthm_#1_env_setup:n  }
      { #2 }
  }
\cs_new:Nn \__zthm_env_setup:nnn
  {
    % NOTE: '#3' has been dropped.
    \exp_args:Nno \use:c
      { zthm_#1_env_setup:n  }
      { #2 }
  }
\NewDocumentCommand{\zthmenvset}{mm}
  {
    \zcmd_if_preamble:TF
      {
        \exp_args:Nc \zthm_envnew_var_init:Nnn
          { g__ztex_thm_name_\g__ztex_lang_math_tl _prop }
          { #1 }{ #2 }
      }{
        \zthm_envnew_var_init:Nnn
    ...
    \keyval_parse:nnn
      { \__zthm_env_setup:nn  {#1} }
      { \__zthm_env_setup:nnn {#1} }
      { #2 }
  }
...
\cs_new_protected:Npn \__ztex_thm_hook_parser_setup:nnn #1#2#3
  {% #1:boolean; #2:hook-type(theorem/proof); #3:env name
    \bool_if:nTF { #1 }
      {
            ...
            \tl_if_empty:nTF { #3 }
              {
                ...
                \hook_gput_code:nen 
                  ...
                  {
                    thm-#2-hook.
                      \int_use:c {g__ztex_thm_#2_hook_index_int}
                  }{ ##2 }
              }{
                ...
                \hook_gput_code:nen 
                  ...
                  {
                    thm-#2-#3-hook.
                      \int_use:c {g__ztex_thm_#2_#3_hook_index_int}
                  }{ ##2 }
              }
          }
      }{
...
\NewDocumentCommand{\zthmhook}{sO{theorem}m}
  {
    \exp_args:Neee \__ztex_thm_hook_parser_setup:nnn
      { \IfBooleanTF{#1}{\c_true_bool}{\c_false_bool} }
      { theorem }
      { \IfValueT{#2}{#2} }
    \keyval_parse:NNn
      \use_none:n
      \__ztex_thm_hook_parser:nn
      { #3 }
  }
...
\hook_gput_code:nnn {ztex/thm-theorem/before}
  {thm-theorem-before-par}{\par}
\hook_gput_code:nnn {ztex/thm-proof/before}
  {thm-proof-before-par}{\par}
\NewDocumentCommand{\zthmbefore}{O{theorem}+m}
  {
    \hook_gremove_code:nn {ztex/thm-#1/before}
      {thm-#1-before-par}
    \hook_gput_code:nnn {ztex/thm-#1/before}
      {thm-#1-before}{#2}
  }
