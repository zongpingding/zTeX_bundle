\ProvidesExplFile{ztex.module.thm.tex}{2025/05/05}{1.0.1}{thm~module~for~ztex}
...
\cs_new_protected:Npn \__ztex_thm_new_style:nn #1#2 {
  \ztex_keys_define:nn { thm/style } {
    ...
    #1 / preamble .code:n    = { 
      ...
      \tl_if_eq:cnT {g__ztex_thm_style_tl}{#1}{
        % \hook_gremove_code:nn {env/document/before}{ztex-thm-preamble}
...
\ztex_msg_set:nn {mathEnv-style}{
  You~use~an~incorrect~MathEnv~style:~'\g__ztex_thm_style_tl',~All~valid~
...
\cs_new:Npn \__ztex_thm_warp_end:n #1 
  {
    \tl_if_exist:cTF {g__ztex_thm_style_\g__ztex_thm_style_tl _end_tl}
      ...
      {\ztex_msg_error:n {mathEnv-style}}
    \__ztex_thm_toc_add:eeoe
      {\g__ztex_thm_toc_level_tl}
      { 
        % \ztexslideTF {\exp_not:N \exp_not:N \exp_not:N \numberline {\zthmnumber}}{}
        \exp_not:N \l__ztex_thm_toc_prefix_tl
        \exp_not:n {\prop_item:Nn \g_ztex_thm_toc_symbols_prop {#1}}
        \g__ztex_thm_theorem_title_tl 
      }
      {\ztexhyperTF{\exp_not:N \hyperlink{page.\thepage}{\thepage}}{\thepage}}
      {#1.\zthmnumber}
  }
...
\cs_new:Npn \__ztex_thm_theorem_title_item:nnn #1#2#3 
  {% #1:env-name; #2:note; #3:separator
    ...
    \cs_set:Npn \zthmname {
      % \textcolor{\tl_use:c {l__ztex_#1_color_tl}}
...
\bool_new:N \g__ztex_thm_toc_bool
\hook_gput_code:nnn { enddocument } { thm-toc }
  {
    \bool_if:NT \g__ztex_thm_toc_bool {
      \iow_new:N \tf@thlist
...
\NewDocumentCommand\zthmtocstop{}
  {
    \bool_gset_false:N \g__ztex_thm_toc_bool
    % \iow_close:N \tf@thlist
    \cs_set:Npn \__ztex_thm_toc_add:nnnn ##1##2##3##4 {}
  }
\cs_new:Npn \__ztex_thm_toc_add:nnnn #1#2#3#4 
  {
    \iow_now:Ne \@auxout 
      {
...
\cs_generate_variant:Nn \__ztex_thm_toc_add:nnnn { eeee, eeoe, nnee, nnoe }
\ztex_keys_define:nn { thm/add }
  {
    % level   .tl_set:N  = \l__ztex_add_thm_toc_level_tl,
...
\NewDocumentCommand{\zthmtoc}{O{1}}
  {
    \bool_gset_true:N \g__ztex_thm_toc_bool
    \group_begin:
