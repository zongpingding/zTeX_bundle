\ProvidesExplFile{zlatex.module.thm.tex}{2024/10/24}{1.0.0}{thm~module~for~zlatex}
...
\zlatexThmStyleNew {
  ...
  background = {
    begin = {
      \def\FrameCommand{\colorbox{\thm@temp@color!10}}
...
\cs_new:Npn \__zlatex_thm_warp_end:n #1 {
  ...
  \__zlatex_thm_toc_add:eeee 
    {\g__zlatex_thm_table_level}{\zlatexThmName\ (\zlatexThmNote)}
    {\thepage}{#1.\zlatexThmNumber}
}
% thm title interface
\NewHook{zlatex/thm/titleformat}
\cs_new:Npn \__zlatex_thm_title_item:nnnn #1#2#3#4 {
  \cs_set:Npn \zlatexThmName  {\prop_item:cn {g__zlatex_thm_name_prop}{#1}}
  \cs_set:Npn \zlatexThmNote  {#2}
  ...
  \tl_gset:Nn \g__zlatex_thm_title_tl {
    #3\zlatexThmName#4\zlatexThmNumber#4(\zlatexThmNote)
  }
...
\cs_generate_variant:Nn \__zlatex_thm_toc_add:nnnn { eeee }
...
\zlatex_keys_define:nn {thm/toc}
  {
    ...
    vspace .dim_gset:N = \g__zlatex_thm_toc_vspace_dim,
    vspace .initial:n  = 10pt,
  }
\NewDocumentCommand{\zlatexThmToc}{o}
  {
    ...
    \par\addvspace{\g__zlatex_thm_toc_vspace_dim}\@starttoc{ thlist }
    \group_end:
  }
...
\NewHook{zlatex/thmstyle/begin}
\NewReversedHook{zlatex/thmstyle/end}
  ...
  \IfBooleanTF{#1}
    {
      \cs_set:Npn \__zlatex_hook_add:nn ##1##2
        {
          ...
          \hook_gput_code:nen {zlatex/thmstyle/##1}
      ...
      \cs_set:Npn \__zlatex_hook_add:nn ##1##2
        {
          \int_gincr:N \g__zlatex_thm_hook_index_int
          \hook_gput_next_code:nn {zlatex/thmstyle/##1}{##2} 
        }
    }
  \keyval_parse:NNn 
    \use_none:n
