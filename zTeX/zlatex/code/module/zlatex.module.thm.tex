\ProvidesExplFile{zlatex.module.thm.tex}{2025/04/21}{1.0.1}{thm~module~for~zlatex}
...
\cs_new_protected:Npn \zlatex_thm_name_set:nn #1#2 {
  \prop_gset_from_keyval:cn {g__zlatex_thm_name_#1_prop} {#2}
}
...
\NewDocumentCommand{\zlatexThmNameSet}{mm}{
  \prop_gput_from_keyval:cn {g__zlatex_thm_name_#1_prop} {#2}
}
...
\tl_new:N \g__zlatex_thm_theorem_title_tl
\def\zlatexThmTitle{\@ifstar\@zlatexThmTitle\@@zlatexThmTitle}
\def\@zlatexThmTitle{\__zlatex_thm_theorem_title:}
\def\@@zlatexThmTitle{\tl_use:N \g__zlatex_thm_theorem_title_tl}
\bool_new:N \g__zlatex_thm_title_inline_bool
\NewDocumentCommand{\zlatexThmTitleSwitch}{s}{
  \IfBooleanTF{#1}
    { \bool_gset_true:N \g__zlatex_thm_title_inline_bool }
    { \bool_gset_false:N \g__zlatex_thm_title_inline_bool}
}
\cs_new:Npn \__zlatex_thm_color_set_check:nn #1#2 
  {
    \clist_clear:N \l_tmpa_clist
    \clist_put_right:NV \l_tmpa_clist \g__zlatex_thm_theorem_clist
    \clist_put_right:NV \l_tmpa_clist \g__zlatex_thm_proof_clist
    \zlatex_msg_set:nn {thm-color-set}{
      Your~color~spec~key~'#1'~is~not~in~the~thm~env~list,~please~check~it~again.
    }
    \clist_if_in:NnF \l_tmpa_clist {#1}
      { \zlatex_msg_error:n {thm-color-set} }
  }
\NewDocumentCommand{\zlatexThmColorSetup}{m}
  {
    % the checker may lower the performance ???
    \zlatex_label_hook_preamble_last:nn {zlatex-thmcolor-setup-user}{
      \keyval_parse:nnn 
        { \use_none:n }
        { \__zlatex_thm_color_set_check:nn }
        { #1 }
      \zlatex_keys_set:nn {color}{#1}
    }
  }
\DeclareHookRule{env/document/before}
  {zlatex-thmcolor-setup-user}{>}{zlatex-thmall-setup-user}
\@onlypreamble\zlatexThmColorSetup
...
\NewDocumentCommand{\zlatexThmCreate}{O{theorem}m}{
...
\zlatex_msg_set:nn {mathEnv-style}{
  ...
  MathEnv~are:'plain',~'leftbar',~'background',~'fancy',~'shadow',~'paris'.
}
% thm counter
...
% thm env warper
\cs_new:Npn \__zlatex_thm_warp_start:nnn #1#2#3 {
  \def\thm@temp@color{\tl_use:c {l__zlatex_#1_color_tl}}
  \def\thm@temp@name{#1}
  \__zlatex_thm_theorem_title_item:nnn {#1}{#2}{#3}
  \tl_use:c {g__zlatex_thm_style_\g__zlatex_thm_style_tl _option_tl}
  \tl_if_exist:cTF {g__zlatex_thm_style_\g__zlatex_thm_style_tl _option_tl}
    {\tl_use:c {g__zlatex_thm_style_\g__zlatex_thm_style_tl _option_tl}}
...
\cs_new:Npn \__zlatex_thm_warp_end:n #1 
  {
    \tl_if_exist:cTF {g__zlatex_thm_style_\g__zlatex_thm_style_tl _end_tl}
      {\tl_use:c {g__zlatex_thm_style_\g__zlatex_thm_style_tl _end_tl}}
      {\zlatex_msg_error:n {mathEnv-style}}
    \__zlatex_thm_toc_add:eeoe
      {\g__zlatex_thm_table_level}
      { 
        % \zlatexSlideTF {\exp_not:N \exp_not:N \exp_not:N \numberline {\zlatexThmNumber}}{}
        \exp_not:N \__zlatex_thm_toc_prefix_tl
        \exp_not:n {\prop_item:Nn \g_zlatex_thm_toc_symbols_prop {#1}}
        \g__zlatex_thm_theorem_title_tl 
      }
      {\zlatexHyperTF{\exp_not:N \hyperlink{page.\thepage}{\thepage}}{\thepage}}
      {#1.\zlatexThmNumber}
  }

% thm theorem title interface
\NewHook{zlatex/thm-theorem/titleformat}
\cs_new:Npn \__zlatex_thm_theorem_title_item:nnn #1#2#3 
    ...
    \bool_if:NTF \g__zlatex_thm_cntshare_bool
      {\cs_set:Npn \zlatexThmNumber {
        \cs:w the\g__zlatex_thm_cntparent_tl\cs_end:
          .\arabic{zlatex@thm@sharecnt}}
        \refstepcounter{zlatex@thm@sharecnt}
      }{\cs_set:Npn \zlatexThmNumber {
        \cs:w the\g__zlatex_thm_cntparent_tl\cs_end:
          .\arabic{#1}}
        \refstepcounter{#1}
      }
    \tl_gset:Nn \g__zlatex_thm_theorem_title_tl {
      \zlatexThmName #3 \zlatexThmNumber
      \tl_if_empty:eF {\zlatexThmNote{}{}}{#3}
      \zlatexThmNote{(}{)} #3
    }
    \UseHook{zlatex/thm-theorem/titleformat}
  }
\cs_new:Npn \__zlatex_thm_theorem_title: 
  {
    \group_begin:
      \noindent\bfseries
      \tl_use:N \g__zlatex_thm_theorem_title_tl
    \group_end:
  }
% thm proof title interface
\tl_new:N \g__zlatex_thm_proof_title_tl
\NewHook{zlatex/thm-proof/titleformat}
\cs_new:Npn \__zlatex_thm_proof_title_item:nn #1#2 
  {% #1:env-name; #2:separator
    \cs_set:Npn \zlatexThmName {
        {\prop_item:cn {g__zlatex_thm_name_prop}{#1}}
      }
    \def\thm@proof@temp@color{\tl_use:c {l__zlatex_#1_color_tl}}
    \tl_gset:Nn \g__zlatex_thm_proof_title_tl {
      \zlatexThmName #2
    }
    \UseHook{zlatex/thm-proof/titleformat}
  }
\cs_new:Npn \__zlatex_thm_proof_title: 
  {
    \group_begin:
      \noindent\bfseries\color{\thm@proof@temp@color}
      \tl_use:N \g__zlatex_thm_proof_title_tl :\,
    \group_end:
  }
% users' interface of thm title format
\tl_new:N \g__zlatex_thm_proof_title_before_tl
\tl_new:N \g__zlatex_thm_theorem_title_before_tl
\tl_gset:Nn \g__zlatex_thm_proof_title_before_tl {\noindent}
\tl_gset:Nn \g__zlatex_thm_theorem_title_before_tl {\noindent}
\NewDocumentCommand{\zlatexThmTitleBefore}{O{theorem}m}{
  \tl_gset:cn {g__zlatex_thm_#1_title_before_tl} {#2}
}
\newcommand{\zlatex@title@before}[1]{
  \tl_use:c {g__zlatex_thm_#1_title_before_tl}
}
\NewDocumentCommand{\zlatexThmTitleFormat}{sO{theorem}m}{
  \IfBooleanTF{#1}{
    \AddToHook{zlatex/thm-#2/titleformat}{
      \cs_set:cpn {__zlatex_thm_#2_title:} 
        {\group_begin:#3\group_end:}
    }
  }{
    \AddToHookNext{zlatex/thm-#2/titleformat}{
      \cs_set:cpn {__zlatex_thm_#2_title:} 
        {\group_begin:#3\group_end:}
    }
  }
}
\@onlypreamble\zlatexThmTitleFormat
...
\NewDocumentCommand\zlatexThmTocStop{}
  {
    \bool_gset_false:N \g__zlatex_thm_toc_bool
    % \iow_close:N \tf@thlist
    \cs_set:Npn \__zlatex_thm_toc_add:nnnn ##1##2##3##4 {}
  }
...
\cs_generate_variant:Nn \__zlatex_thm_toc_add:nnnn { eeee, eeoe, nnee, nnoe }
...
\NewDocumentCommand{\zlatexThmTocAdd}{O{section}m}{
  \group_begin:
  \zlatex_keys_set:nn {thm/add}{#2}
  \__zlatex_thm_toc_add:nnoe {#1}
    ...
    \clist_map_inline:Nn \g__zlatex_thm_theorem_clist 
      {
        ...
        \exp_after:wN \def\cs:w the#1\cs_end:
          {\cs:w the\g__zlatex_thm_cntparent_tl\cs_end:.\arabic{#1}}
        \__zlatex_cref_math_env:n {#1}
        \DeclareDocumentEnvironment{#1}{O{}}
          {
            \UseHook{zlatex/thm-theorem/before} \UseHook{zlatex/thm-theorem-#1/before}
            \__zlatex_thm_warp_start:nnn {#1}{##1}{\ }
            \MakeLinkTarget*{#1.\zlatexThmNumber}
            \bool_if:NT \g__zlatex_thm_title_inline_bool { 
              \group_begin: 
              \zlatex@title@before{theorem}\__zlatex_thm_theorem_title:
              \group_end:
            }   
            \UseHook{zlatex/thm-theorem/begin} \UseHook{zlatex/thm-theorem-#1/begin}
            \tl_trim_spaces:n            
          }{
            \UseHook{zlatex/thm-theorem/end} \UseHook{zlatex/thm-theorem-#1/end}
            \__zlatex_thm_warp_end:n {#1}
            \UseHook{zlatex/thm-theorem/after} \UseHook{zlatex/thm-theorem-#1/after}
          }
      }
  }
% proof-like env
    ...
    \clist_map_inline:Nn \g__zlatex_thm_proof_clist 
      {
        \DeclareDocumentEnvironment{#1}{O{}}
          {
            \UseHook{zlatex/thm-proof/before} \UseHook{zlatex/thm-proof-#1/before}
            \__zlatex_thm_proof_title_item:nn {#1}{\,} 
            \group_begin:
              \zlatex@title@before{proof} \__zlatex_thm_proof_title:
            \group_end:
            \UseHook{zlatex/thm-proof/begin} \UseHook{zlatex/thm-proof-#1/begin}
            \tl_set:Nn \l__thm_proof_name_tl {#1} 
            \tl_trim_spaces:n
          }{
            \UseHook{zlatex/thm-proof/end} \UseHook{zlatex/thm-proof-#1/end}
            \str_if_eq:VnTF \l__thm_proof_name_tl {proof}{\hfill\qedsymbol\par}{\par}
            \UseHook{zlatex/thm-proof/after} \UseHook{zlatex/thm-proof-#1/after}
          }
      }
  }
...
% general thm hook
\NewHook{zlatex/thm-theorem/before}
\NewHook{zlatex/thm-theorem/begin}
\NewReversedHook{zlatex/thm-theorem/end}
\NewReversedHook{zlatex/thm-theorem/after}
\NewHook{zlatex/thm-proof/before}
\NewHook{zlatex/thm-proof/begin}
\NewReversedHook{zlatex/thm-proof/end}
\NewReversedHook{zlatex/thm-proof/after}
\int_new:N \g__zlatex_thm_proof_hook_index_int
\int_new:N \g__zlatex_thm_theorem_hook_index_int
\int_gzero:N \g__zlatex_thm_proof_hook_index_int
\int_gzero:N \g__zlatex_thm_theorem_hook_index_int

% specific thm hook
\clist_map_inline:nn {theorem, proof}
  {
    \clist_map_inline:cn {g__zlatex_thm_#1_clist} 
      {
        \NewHook{zlatex/thm-#1-##1/before}
        \NewHook{zlatex/thm-#1-##1/begin}
        \NewReversedHook{zlatex/thm-#1-##1/end}
        \NewReversedHook{zlatex/thm-#1-##1/after}
        \int_new:c {g__zlatex_thm_#1_##1_hook_index_int}
        \int_gzero:c {g__zlatex_thm_#1_##1_hook_index_int}
      }
  }
\cs_generate_variant:Nn \hook_gput_code:nnn {ne}
\cs_new_protected:Npn \__zlatex_thm_hook_add:nnn #1#2#3
  {% #1:if-star; #2:hook-type(theorem/proof); #3:env name; 
    \IfBooleanTF{#1}
      {
        \cs_set:Npn \__zlatex_thm_hook_parser:nn ##1##2
          {
            \IfValueTF{#3}{
              \int_gincr:c {g__zlatex_thm_#2_#3_hook_index_int}
              \hook_gput_code:nen {zlatex/thm-#2-#3/##1}
                {thm-#2-#3-hook.\int_use:c {g__zlatex_thm_#2_#3_hook_index_int}}
                {##2}
            }{
              \int_gincr:c {g__zlatex_thm_#2_hook_index_int}
              \hook_gput_code:nen {zlatex/thm-#2/##1}
                {thm-#2-hook.\int_use:c {g__zlatex_thm_#2_hook_index_int}}
                {##2}
            }
          }
      }{
        \cs_set:Npn \__zlatex_thm_hook_parser:nn ##1##2
          {
            \IfValueTF{#3}{
              \int_gincr:c {g__zlatex_thm_#2_#3_hook_index_int}
              \hook_gput_next_code:nn {zlatex/thm-#2-#3/##1}{##2}
            }{
              \int_gincr:c {g__zlatex_thm_#2_hook_index_int}
              \hook_gput_next_code:nn {zlatex/thm-#2/##1}{##2} 
            }
          }
      }
  }
...
\NewDocumentCommand{\zlatexThmHook}{sO{theorem}m}{
  \__zlatex_thm_hook_add:nnn {#1}{theorem}{#2}
  \keyval_parse:NNn 
    \use_none:n
    \__zlatex_thm_hook_parser:nn {#3}
}
\NewDocumentCommand{\zlatexThmProofHook}{sO{proof}m}{
  \__zlatex_thm_hook_add:nnn {#1}{proof}{#2}
  \keyval_parse:NNn 
    \use_none:n
    \__zlatex_thm_hook_parser:nn {#3}
}
\hook_gput_code:nnn {zlatex/thm-theorem/before}{thm-theorem-before-par}{\par}
\hook_gput_code:nnn {zlatex/thm-proof/before}{thm-proof-before-par}{\par}
\NewDocumentCommand{\zlatexThmBefore}{O{theorem}+m}{
  \hook_gremove_code:nn {zlatex/thm-#1/before}{thm-#1-before-par}
  \hook_gput_code:nnn {zlatex/thm-#1/before}{thm-#1-before}{#2}
}
\@onlypreamble\zlatexThmBefore



% ==> zlatex thm hooks seq order
\DeclareHookRule{env/document/before}
