...
\ztex_msg_set:nn { fontcfg / lang }
  { Current~font~type~supported~are:'en',~'CJK'. }
    ...
    \clist_map_inline:nn { #1 }
      {
        \keyval_parse:nnn
          { \use_none:n }
          { \__ztex_fontcfg_setfamily_aux:nnn {##1} }
          { #2 }
      }
  }
\ztex_msg_set:nn { fontcfg / family }
  { Valid~family~options~are:'main',~'sans'~and~'mono'. }
\cs_set:Npn \__ztex_fontcfg_setfamily_aux:nnn #1#2#3
  {% #1:lang, #2:family, #3:{font}{font features}
    \tl_if_in:nnF {en, CJK}{#1}
      { \ztex_msg_error:n { fontcfg / lang } }
    \tl_if_in:nnF {main, sans, mono}{#2}
      { \ztex_msg_error:n { fontcfg / family } }
    \ltx_main_fontfamily_set:eee
      { \str_case:nn {#1}{{en}{}{CJK}{CJK}} #2 }
      { \use_i:nn #3  }
      { \use_ii:nn #3 }
  }
\cs_new_protected:Npn \ltx_main_fontfamily_set:nnn #1#2#3
  {
    \cs:w set #1 font\cs_end:
      { #2 }[ #3 ]
  }
\cs_generate_variant:Nn \ltx_main_fontfamily_set:nnn
  { eee }