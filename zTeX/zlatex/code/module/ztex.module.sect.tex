\ProvidesExplFile{ztex.module.sect.tex}{2025/08/22}{1.0.1}{sect~module~for~ztex}
...
\cs_new:Npn \__ztoc_ignore_negate_parser:nnn #1#2#3
  {
    ...
    % TODO: handle special section name, eg: '\textbf{1.1-2}'
...
% The implementation of '\__ztoc_dotted_tocline:nnn' comes afterward.
...
\cs_new:Npn \__ztoc_dotted_tocline_raw:nnn #1#2#3
  {
    ...
    \edef\ztoccurname { \__ztoc_extract_name:w #2\scan_stop: }
    \edef\ztoccurtitle{ \__ztoc_extract_title:w #2\scan_stop: }
...
% TODO: implement '\__zsect_local_toc_generate:nn' to generate variables
%       '\g_ztoc_keyval<toc|lof>_seq' from a user-specified file.
\cs_new:Npn \__zsect_local_toc_generate:nn #1#2
  {% #1:toc, lof, etc; #2:file
    % generate '*.ptoc'
    % ...
    % generate seq from '*.ptoc'
    \ztool_gread_file_as_seq:nnc { \c_false_bool }
      { #1.p##1 }
      { g_ztoc_keyval##1_seq }
  }
    ...
    \seq_gput_right:Ne \g__ztoc_keyvaltoc_seq
      {
        ...
        name   = { \zsect@tocnum },
        title  = { \tl_if_empty:nTF {#1}{#2}{#1} },
        page   = { \thepage },
        raw    = { \token_to_str:N \contentsline 
                  { \l__zsect_title_class_tl }
                  { 
                    { \zsect@tocnum }
                    {
...
\cs_generate_variant:Nn \zsect_add_table_line:nnnn  { eeee }
\cs_generate_variant:Nn \zsect_add_figure_line:nnnn { eeee }
...
\long\def\@caption#1[#2]#3 
  {
    \par \use:c { zsect_add_#1_line:eeee } 
      { #1 }
...
% ----------------------------------------------------------------------
% ==> marker interface
\mark_new_class:n { ztex-1st   } % for top title class ('part')
\mark_new_class:n { ztex-left  }
\mark_new_class:n { ztex-right }
\mark_new_class:n { ztex-right-nonempty }
\mark_new_class:n { ztex-4th   } % for the 4th level title class ('subsection'/'subsubsection')
\cs_new:Npn \zsect_mark_insert:nn #1#2
  {
    \exp_args:Ne \int_case:nnF { \prop_item:Nn \c_zsect_level_prop {#1} }
      {
        ...
        \ztex_msg_set:nn { mark-nonexist }
          { current~title~class:'#1'~is~not~a~valid~mark~class. } 
        \ztex_msg_error:n { mark-nonexist } 
      }
  }
\cs_new:Npn \zsect_right_mark_insert:n #1
  {
...
\cs_new:Npn \__zsect_lower_markclass_empty:n #1 
  {
    \prop_map_inline:Nn \c_zsect_level_prop 
      {
...
\cs_generate_variant:Nn \zsect_mark_insert:nn { Vn, ee }
\cs_generate_variant:Nn \__zsect_lower_markclass_empty:n { V, o, e }
...
\cs_set_eq:NN \ztexrightmark \zsect_robust_left_mark:
\cs_set_eq:NN \robustleftmark  \zsect_robust_right_mark:
\cs_set_eq:NN \robustrightmark \zsect_robust_right_mark:
\ztex_msg_set:nn { right-mark-empty }{ right~mark~is~empty~at~page:\thepage. }
...
\DeclareTemplateCode{ztexsect}{default}{3}
  {
    ...
    % mark and toc
    \zsect_mark_insert:Vn \l__zsect_title_class_tl { #2 }
    % \__zsect_lower_markclass_empty:V \l__zsect_title_class_tl
    \IfBooleanTF{#3}{}
      {
        % NOTE:
        % 1. set '\Hy@pdfstring' true to support command '\texorpdfstring';
        % 2. non-empty toc name(#1) comes before the sec name(#2);
        % 3. command or math shift may occur in '#1'(i.e. toc name).
        {
          \bool_if:NT \g__ztex_hyperref_bool { \Hy@pdfstringtrue }
          \exp_args:Ne \__zsect_title_bookmark_add:n 
            {
              \tl_if_empty:nTF { #1 }
                { #2 }{ #1 }
            }
    ...
    \tl_clear:N \l__zsect_title_pagestyle_tl
  }
