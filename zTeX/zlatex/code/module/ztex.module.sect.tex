...
\newcommand{\ztocindexall}[3][toc]
  {
    \ztoc_table_tocline_index_bynametitle:nnc
      { #2 }{ #3 }
      { g_ztoc_keyval#1_seq }
  }
% check if first contentsline(expandable)
...
% check if last contentsline(expandable)
\prg_new_conditional:Npnn \ztoc_if_last_tocline:Nnn #1#2#3 { p, T, F, TF }
  {% #1:ori seq; #2:name; #3:title
    \int_compare:nNnT 
      { \ztoc_table_tocline_index_bynametitle:nnN {#2}{#3}#1 }
        < 
      { 0 }
      { \prg_return_false: }
        ...
        \ztoc_get_class_level_expandable:e 
          {
            \prop_item:en 
              { 
                \seq_item:Ne #1 
                  { \ztoc_table_tocline_index_bynametitle:nnN {#2}{#3}#1 + 1 } 
              }
              { class }
          } + 0
      }
...
\prg_generate_conditional_variant:Nnn \ztoc_if_last_tocline:Nnn
  { c, Nee, cee }{ p, T, F, TF }
\NewDocumentCommand{\ztociflast}{O{toc}mm}
  {
...
\cs_new:Npn \__ztoc_dotted_tocline_raw:nnn #1#2#3
  {
    \ifnum #1 > \c@tocdepth \else
      ...
      \cs_gset:Npn \ztocdepth { #1 }
      \cs_gset:Npe \ztocclass { \ztoc@current@class }
      \cs_gset:Npn \ztocpage  { #3 }
...
\cs_new:Npn \ztoc_table_tocline_index_bynametitle:nnN #1#2#3
  {% #1:name; #2:title; #3:ori seq
    \int_eval:n {
      (
        \seq_map_tokens:Nn #3
          {
            \__ztoc_table_tocline_index_bynametitle_aux:nnn
              { #1 }{ #2 }
          } + 0
      )
...
\cs_new:Npn \__ztoc_table_tocline_index_bynametitle_aux:nnn #1#2#3
  {
    \bool_lazy_all:nTF
      {
        { \exp_args:Ne \str_if_eq_p:nn
...
\cs_generate_variant:Nn \ztoc_table_tocline_index_bynametitle:nnN
  { nnc, eec, eeN }
