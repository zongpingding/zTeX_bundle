\ProvidesExplFile{ztex.module.sect.tex}{2025/07/08}{1.0.1}{sect~module~for~ztex}
...
\cs_new:Npn \__ztoc_dotted_tocline:nnn #1#2#3
  {
    \ifnum #1 > \c@tocdepth \else
      ...
      \ztoc_group_hook_add:n {\ztoc@current@class,\__use_toc_group_int:e {\ztoc@current@class},begin}
...
\cs_new:Npn \__ztoc_dotted_tocline_raw:nnn #1#2#3
  {
      ...
      }
    {
      ...
      \__ztoc_leader_set: % leaders
      \nobreak \__ztoc_page_set:nn { #3 }{page.#3} \l__ztoc_line_end_tl
...
\ztex_keys_define:nn { ztoc/option }
  { 
    ...
    ignore.level   .code:n = { \def\ztoc@ignore@level {#1} },
...
\NewDocumentCommand{\ztocgroupinsert}{mm}
  {
    \AddToHook{#1}{#2}
  }
...
% '\multitableofcontent' need 'multicol' package
...
\cs_new:Npn \__zsect_title_bookmark_add:n #1
  {
    \zsect_bookmark_add:eee
      ...
      { \l__zsect_title_bookmark_before_tl #1 \l__zsect_title_bookmark_after_tl }
