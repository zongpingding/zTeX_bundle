%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% This is file 'ztex.module.sect.tex' of ztex bundle.           %
%% The following code is experimental and will likely change.    %
%% Copyright 2024, 2025, 2026 Zongping Ding.                     %
%                                                                %
...
\cs_new:Npn \zsect_package_disable_error:
  {
    ...
    \@ifpackageloaded{ multitoc }{ \__zsect_package_disable_error: }{}
    \@ifpackageloaded{  minitoc }{ \__zsect_package_disable_error: }{}
...
\clist_const:Nn \c_ztoc_table_types_clist
  { 
    ...
    tog,  % table of glossary('.log' is already used.)
...
% ----------------------------------------------------------------------
% NOTE:
% 1. only 'type' has prefix 'ztex'.
% 2. private cmds like '\__template_use_instance:nn' may be used in the future.
\cs_new_protected:Npn \ztex_new_templatetype:nn #1#2
  { 
    \NewTemplateType{ztex#1}{#2}
  }
\cs_new_protected:Npn \ztex_declare_template_interface:nnnn #1#2#3#4
  {
    \DeclareTemplateInterface{ztex#1}{#2}
      {#3}{#4}
  }
\cs_new_protected:Npn \ztex_declare_template_code:nnnnn #1#2#3#4#5
  {
    \DeclareTemplateCode{ztex#1}{#2}
      {#3}{#4}{#5}
  }
\cs_new_protected:Npn \ztex_declare_template_copy:nnn #1#2#3
  {
    \DeclareTemplateCopy{ztex#1}
      {#2}{#3}
  }
\cs_new_protected:Npn \ztex_declare_instance:nnnn #1#2#3#4
  {
    \DeclareInstance{ztex#1}{#2}
      {#3}{#4}
  }
\cs_new_protected:Npn \ztex_declare_instance_copy:nnn #1#2#3
  {
    \DeclareInstanceCopy{ztex#1}{#2}{#3}
  }
\cs_new_protected:Npn \ztex_use_instance:nn #1#2
  {
    \UseInstance{ztex#1}{#2}
    % <argument>
  }
\cs_new_protected:Npn \ztex_use_template:nnn #1#2#3
  {
    \UseTemplate{ztex#1}{#2}
      { #3 } 
      % <argument>
  }
\cs_new_protected:Npn \ztex_edit_instance:nnn #1#2#3
  {
    \EditInstance{ztex#1}{#2}{#3}
  }
\cs_new_protected:Npn \ztex_edit_template_defaults:nnn #1#2#3
  {
    \EditTemplateDefaults{ztex#1}
      {#1}{#3}
  }

% get templates & instances info(debug)
\cs_new_protected:Npn \ztex_show_instance_values:nn #1#2
  {
    \ShowInstanceValues{ztex#1}{#2}
  }
\cs_new_protected:Npn \ztex_show_template_code:nn #1#2
  {
    \ShowTemplateCode{ztex#1}{#2}
  }
\cs_new_protected:Npn \ztex_show_template_defaults:nn #1#2
  {
    \ShowTemplateDefaults{ztex#1}{#2}
  }
\cs_new_protected:Npn \ztex_show_template_interface:nn #1#2
  {
    \ShowTemplateInterface{ztex#1}{#2}
  }
\cs_new_protected:Npn \ztex_show_template_variables:nn #1#2
  {
    \ShowTemplateVariables{ztex#1}{#2}
  }
...
%                           bookmark interface
% ----------------------------------------------------------------------
% TODO: '\DocumentMetadata' will make all bookmark targets wrong.
...
% NOTE: 
% 1. re-def these commands at last to prevent them from being modified;
...
\cs_new_protected:Npn \zsect_hang_from:n #1
  {
    \setbox\@zsectmpboxa\hbox{{#1}}
    \hangindent\wd\@zsectmpboxa
    \noindent\box\@zsectmpboxa
  }
\cs_new_protected:Npn \zsec_normal_text:n #1
  {
    {\normalsize\normalfont\normalcolor #1}
  }
    ...
    { en }
      {
        \ztex_hook_preamble_last:n
          {
            \cs_set_nopar:Npn \figurename{Figure}
            \cs_set_nopar:Npn \tablename{Table}
            \cs_set_nopar:Npn \lstlistingname{Listing}
          }
      }
    { cn }
      {
        \ztex_hook_preamble_last:n
          {
            \cs_set_nopar:Npn \figurename{图}
            \cs_set_nopar:Npn \tablename{表}
            \cs_set_nopar:Npn \lstlistingname{代码} 
          }
      }
  }{ \relax }

% special '\caption' to record contents
    ...
    \clist_if_in:NnTF \c_ztoc_special_level_clist
      { #1 }{ \def\@captype{#1} }
      {
        \ztex_msg_set:nn {wrong@caption@type}
          { 
            caption~type~'#1'~is~not~permitted,~
            use~'figure',~'table',~etc~instead.
          }
...
\cs_generate_variant:Nn \zsect_caption_use:nnn
  { ooo, eee }
\NewDocumentCommand{\zseccaption}
...
\cs_new_protected:Npn \zsect_ltx_float_setup:nnnnnn #1#2#3#4#5#6
...
\ztex_declare_template_code:nnnnn {toc}{default}{3}
  {
    ...
    enhance        = \l__ztoc_public_macro_enhance_bool,
    ...
    %   #3: page
    %%% NOTE:
    % 1. use '\ztoc@current@class' to get current class.
    % 2. group wraper has been removed.
    \AssignTemplateKeys
    \__ztoc_public_macro_setup:nnn 
      { #1 }{ #2 }{ #3 }
    \ztoc_group_hook_create:Nnnn 
      \__ztoc_dotted_tocline:nnn 
      { #1 }{ #2 }{ #3 }
  }

% ==> toc public macros setup
% NOTE: these basic macros do NOT cost too much time
\cs_new:Nn \__ztoc_public_macro_setup:nnn
  {
    \cs_gset_nopar:Npn \ztocdepth  { #1 }
    \cs_gset_nopar:Npe \ztocanchor { \@contentsline@destination }
    \cs_gset_nopar:Npe \ztoctype   { numbered } % TODO: handle this macro.
    \cs_gset_nopar:Npe \ztocclass  { \ztoc@current@class }
    \cs_gset_nopar:Npn \ztocpage   { #3 }
    \cs_gset_protected:Npn \ztoclink ##1
      {
        \exp_args:No \ztexlink
          { \@contentsline@destination }
          { ##1 }
      }
    \cs_gset:Npn \ztocifnumbered ##1##2
      {
        \tl_if_empty:eTF 
          { \__ztoc_extract_name:w #2\scan_stop: }
          { ##2 }{ ##1 }
      }
    \edef\ztoc@expnot@nametitle
      { \zcmd_map_expnot_tl:n {#2} }
    \exp_args:NNe \cs_gset:Npn \ztocname
      {
        \exp_after:wN \__ztoc_extract_name:w 
          \ztoc@expnot@nametitle \scan_stop:
      }
    \exp_args:NNe \cs_gset:Npn \ztoctitle
      {
        \exp_after:wN \__ztoc_extract_title:w 
          \ztoc@expnot@nametitle \scan_stop:
      }
  }
% NOTE: these advance macro only available in 'explicit' code,
%       for that they will cost too much time if call them 
%       for every tocline. We use an bool to control it.
\cs_new:Nn \__ztoc_public_macro_advance_setup:nnn
  {
    \cs_gset_nopar:Npe \ztoctheindex
      {
        \exp_args:Nee \ztocindex{\ztoc@current@class}
          {
            \__ztoc_extract_title:w 
              #2 \scan_stop:
          }
      }
    \cs_gset_nopar:Npe \ztoctheabsindex 
      {
        \exp_args:NNee \ztocindex*{\ztoc@current@class}
          {
            \__ztoc_extract_title:w
              #2 \scan_stop:
          }
      }
    \cs_gset_nopar:Npe \ztocthecard
      {
        \exp_args:Nee \ztoccard
          { \ztoc@current@class }
          {
            \__ztoc_extract_title:w 
              #2 \scan_stop:
    ...
    \cs_gset_nopar:Npe \ztocthehilevel
      { \ztochilevel{toc} }
    \cs_gset_nopar:Npe \ztocthelolevel
      { \ztoclolevel{toc} }
  }
...
\cs_new:Npn \__ztoc_dotted_tocline_raw:nnn #1#2#3
  {
    \ifnum #1 > \c@tocdepth \else
      ...
      \bool_if:NTF \l__ztoc_explicit_bool
        {
          \bool_if:NT \l__ztoc_public_macro_enhance_bool
            {
              \__ztoc_public_macro_advance_setup:nnn
                { #1 }{ #2 }{ #3 }
            }
          \l__ztoc_code_tl
          \bool_set_false:N \l__ztoc_public_macro_enhance_bool
        }{
          {
            ...
            \null\nobreak \hskip -\leftskip
            {
              \__ztoc_name_title_typeset:nn { #2 }
                { \@contentsline@destination }
            } \nobreak
...
% \cs_new:Npn \ltx_ztoc_extract_name:w \numberline #1\scan_stop:
%   { \tl_item:nn {#1}{1} }
% \cs_new:Npn \ltx_ztoc_extract_title:w \numberline #1\scan_stop:
%   { \tl_range:nnn {#1}{2}{-1} }
...
\ztex_declare_instance:nnnn {toc}{ztoc/level 1}{default}
  {
...
\ztex_declare_instance:nnnn {toc}{ztoc/level 2}{default}
  {
    format         = \bfseries,
    name.width     = 1.5em,
    space.before   = 1em\@plus\p@,
...
\ztex_declare_instance:nnnn {toc}{ztoc/level 3}{default}
  {
    name.width     = 2.3em,
    space.hang     = 2.3em,
    space.left     = 3.8em,
  }
\ztex_declare_instance:nnnn {toc}{ztoc/level 4}{default}
  {
    name.width     = 3.2em,
    space.hang     = 3.2em,
    space.left     = 7em,
  }
\ztex_declare_instance:nnnn {toc}{ztoc/level 5}{default}
  {
    name.width     = 4.1em,
    space.hang     = 4.1em,
    space.left     = 11.1em,
  }
\ztex_declare_instance:nnnn {toc}{ztoc/level 6}{default}
  {
    name.width     = 5em,
    space.hang     = 5em,
    space.left     = 16.2em,
  }
\ztex_declare_instance:nnnn {toc}{ztoc/level 7}{default}
  {
    name.width     = 6em,
    space.hang     = 6em,
    space.left     = 22.25em,
...
\cs_new_protected:Npn \ztoc_special_lcmd_setup:nn #1#2
  {% #1:class; #2:level(int)
    \ztex_declare_instance_copy:nnn { toc }
      { ztoc/#1 }{ ztoc/level #2 }
    \ztex_label_hook_preamble_last:nn
        ...
        \zsect_add_toc_line:nnnn
          ...
          { 
            ...
            }
            {
              ...
              \exp_args:Nne \tl_if_empty:oTF { #2 } % add 'o'-expand to test if '\zsectocname' etc are empty
                { #3 }{ #2 }
            }
          }
          { \thepage }
          { \ztexhyperTF {#1.\zsect@tocHnum}{} }
      }
    \zsect_restore_protect:
  }
...
%% programming layer
% >> toc group parser
\cs_new_protected:Nn \ztoc_group_parser:NNNnnnnn
  {% #1:keyval seq; #2:cmd; #3:tl(saved in);
   % #4:before; #5:begin: #6:end; #7:after;
   % #8:step code. 
    \seq_map_inline:Nn #1
      {
        \edef\ztoc@gparser@curclass
          {
            \prop_item:nn {##1}{ class }
          }
        \exp_args:No \__ztoc_step_toc_group_int:n 
          { \ztoc@gparser@curclass }
        \ztoc_get_class_level:No \ztoc@gparser@newclass@level 
          { \ztoc@gparser@curclass }
        \cs_set_eq:NN \gparserclass\ztoc@gparser@curclass
        \cs_set:Npe \gparseranchor
          {
            \prop_item:nn {##1}{ class }
            .
            \prop_item:nn {##1}{ name }
          }
        \cs_set:Npn \gparserindex
          {
            ...
            ( 
              \int_compare_p:n 
                { 
                  ...
                  >= \ztoc@gparser@newclass@level
                }
            )
          }{
            \seq_gpop:NN \g__ztoc_gparser_curstack_seq \l__ztoc_gparser_prev_tl
            \tl_gput_right:Ne #3 {#6} % end
            \tl_gput_right:Ne #3 {#7} % after
          }
        \tl_gput_right:Ne #3 {#4} % before
        \edef\ztoc@gpaser@cmdargs
          {
            {
              \ztoc@gparser@curclass
            }
        ...
        \edef\ztoc@gpaser@cmdargs
          { 
            \exp_args:No \zcmd_map_expnot_tl:n 
              { \ztoc@gpaser@cmdargs }
          }
        \exp_after:wN \def \exp_after:wN 
          \ztoc@gparser@line \exp_after:wN
            {
              \exp_after:wN #2
              \ztoc@gpaser@cmdargs
            }
        \exp_args:NNe \tl_gput_right:Nn #3
          { \ztoc@gparser@line }
        % insert step code
        #8
        \tl_gput_right:Ne #3 {#5} % begin
...
\cs_generate_variant:Nn \ztoc_group_parser:NNNnnnnn
  { ccc, NNNeeeee, ccceeeee }
...
\cs_new_protected:Npn \ztoc_restore_main_table:n #1
  {
    \str_if_eq:eeF { #1 }{ \c_sys_jobname_str }
      {
        \ztoc_maintable_seq_restore:n { toc }
      }
  }
...
\cs_new_protected:Npn \ztoc_generate_table_seq:nn #1#2
  {% #1:file(no ext); #2:type(toc, lof, etc)
    \bool_lazy_and:nnT
      { ! \str_if_eq_p:ee  { #1 }{ \c_sys_jobname_str } }
      { ! \file_if_exist_p:n { #1.#2 } }
      {
        \ztex_msg_set:nn { tablefile@nonexist }
          { 
            custom~table~file~'#1.#2'~does~NOT~exist,~
            check~the~file~name~again.
          }
        \ztex_msg_error:n { tablefile@nonexist }
      }
...
\seq_new:N \g__ztoc_maintable_tmp_seq
\seq_gclear:N \g__ztoc_maintable_tmp_seq
\seq_new:N \g__ztoc_maintable_keyvaltmp_seq
\seq_gclear:N \g__ztoc_maintable_keyvaltmp_seq
% NOTE: the 'save' function should before '\ztoc_generate_table_seq'
\cs_new_protected:Npn \ztoc_maintable_seq_save:n #1
  {
    \seq_gset_eq:Nc \g__ztoc_maintable_tmp_seq 
      { g_ztoc_#1_seq }
    \seq_gset_eq:Nc \g__ztoc_maintable_keyvaltmp_seq 
      { g_ztoc_keyval#1_seq }
  }
\cs_new_protected:Npn \ztoc_maintable_seq_restore:n #1
  {
    \seq_gset_eq:cN { g_ztoc_#1_seq }
      \g__ztoc_maintable_tmp_seq
    \seq_gset_eq:cN { g_ztoc_keyval#1_seq }
      \g__ztoc_maintable_keyvaltmp_seq
    \seq_gclear:N \g__ztoc_maintable_tmp_seq
    \seq_gclear:N \g__ztoc_maintable_keyvaltmp_seq
  }
...
% 'byclass' filter variables setup:
\int_new:N \l__ztoc_tmpa_int
\prop_new:N \l__ztoc_tmpa_prop
\prop_new:N \l__ztoc_tmpb_prop
...
% only count sub-tocline by key(similar to filter but expandable)
\cs_new:Npn \ztoc_tocline_card:nnN #1#2#3
  {
    \int_compare:nNnTF
      {
          ...
          \exp_args:Ne \tl_map_tokens:nn
            {
              ...
              \__ztoc_tocline_card_byclasstitle:nnnn
                { #1 }{ #2 }{ #3 }
            } + 0
        }
      }
  }
\cs_new:Nn \__ztoc_tocline_card_byclasstitle:nnnn
  {
    \int_compare:nNnTF
      {
        \ztoc_get_class_level_expandable:e
...
\cs_generate_variant:Nn \ztoc_tocline_card:nnN
  { oo, ee, nnc, eec }

% high/low level int of current table
\cs_new:Npn \ztoc_tocline_hilevel:N #1
  {
    \exp_args:Ne \zcmd_floats_min:n
      {
        \ztoc_tocline_level_data:N #1
      }
  }
\cs_new:Npn \ztoc_tocline_lolevel:N #1
  {
    \exp_args:Ne \zcmd_floats_max:n
      {
        \ztoc_tocline_level_data:N #1
      }
  }
\cs_new:Npn \ztoc_tocline_level_data:N #1
  {
    \seq_map_function:NN #1
      \__ztoc_tocline_hilo_level_aux:n
  }
\cs_generate_variant:Nn \ztoc_tocline_level_data:N
  { c }
\cs_generate_variant:Nn \ztoc_tocline_hilevel:N
  { c }
\cs_generate_variant:Nn \ztoc_tocline_lolevel:N
  { c }
\cs_new:Nn \__ztoc_tocline_hilo_level_aux:n
  {
    {\ztoc_get_class_level_expandable:e
      {
        \prop_item:nn { #1 }{ class }
...
% extract custom key by class
\cs_new_protected:Npn \ztoc_table_filter_key_byclass:nnnNN #1#2#3#4#5 
  {% #1:class; #2:index; #3:type; #4:raw seq; #5:res seq
    \seq_gclear:N #5
    \bool_set_false:N \l__ztoc_filter_item_found_bool
    \seq_map_inline:Nn #4
      {
        \prop_set_from_keyval:Nn \l__ztoc_tmpa_prop { ##1 }
        \exp_args:Ne \__ztoc_step_filter_int:n 
          { \prop_item:Nn \l__ztoc_tmpa_prop {class} }
        ...
        \exp_args:Ne \int_compare:nNnT 
          { \__ztoc_use_filter_int:n {#1} } = {#2}
          {
            ...
            \seq_gput_right:Ne #5
              { \prop_item:Nn \l__ztoc_tmpa_prop {#3} }
          }
      }
    \__ztoc_reset_filter_int:
  }
\cs_generate_variant:Nn \ztoc_table_filter_key_byclass:nnnNN
  { ooo, eee, nnncc, eeecc }

% extract 'raw' by class
...
% extract by 'name' & 'title'(i.e., {1.2.4}{AAA})
% NOTE: name and title need to be expanded before grabbed.
\cs_new_protected:Npn \ztoc_table_filter_bynametitle:nnNN #1#2#3#4
  {% #1:name; #2:title; #3:ori seq; #4:prop
    \prop_gclear:N #4
    \seq_map_inline:Nn #3
      {
        \prop_set_from_keyval:Nn \l__ztoc_tmpb_prop { ##1 }
        \int_incr:N \l__ztoc_tmpa_int
        \bool_lazy_all:nT
          {
            { \exp_args:Ne \str_if_eq_p:nn
                { \prop_item:Nn \l__ztoc_tmpb_prop {name} }
                { #1 }
            }
            { \exp_args:Ne \str_if_eq_p:nn 
                { \prop_item:Nn \l__ztoc_tmpb_prop {title} }
                { #2 }
            }
          }{
            \prop_gset_eq:NN #4 \l__ztoc_tmpb_prop
          }
      }
  }
\cs_generate_variant:Nn \ztoc_table_filter_bynametitle:nnNN
  { ee, nncc, eecc }

% get index of current contentsline(expandable)
% NOTE: if not found, an negative value will be returned
\cs_new:Npn \__special_seq_map_break:
  { \prg_map_break:Nn \__special_seq_map_break: { } }

% get index by class & name
\cs_new:Npn \ztoc_tocline_index_byclasstitle:nnnN #1#2#3#4
  {% #1:class; #2:title; #3:bool; #4:ori seq
    \int_eval:n {
      (
        \seq_map_tokens:Nn #4
          {
            \__ztoc_tocline_index_byclasstitle_aux:nnnn
              { #1 }{ #2 }{ #3 }
          } + 0
      )
      \prg_break_point:Nn \__special_seq_map_break: {}
    }
  }
\cs_new:Nn \__ztoc_tocline_index_byclasstitle_aux:nnnn
  {
    \bool_lazy_all:nTF
      {
        { \exp_args:Ne \str_if_eq_p:nn
            { \prop_item:nn { #4 }{class} }
            { #1 }
        }
        { \exp_args:Ne \str_if_eq_p:nn 
            { \prop_item:nn { #4 }{title} }
            { #2 }
        }
      }{
        -1 \prg_map_break:Nn \__special_seq_map_break: 
          { )*(-1) }
      }{
        \bool_if:nTF { #3 }
          {
...
\cs_generate_variant:Nn \ztoc_tocline_index_byclasstitle:nnnN
  { nnnc, eeec, eeeN }

% get index by name & title
\cs_new:Npn \ztoc_tocline_index_bynametitle:nnN #1#2#3
  {% #1:name; #2:title; #3:ori seq
    \int_eval:n {
      (
        \seq_map_tokens:Nn #3
          {
            \__ztoc_tocline_index_bynametitle_aux:nnn
              { #1 }{ #2 }
          } + 0
      )
      \prg_break_point:Nn \__special_seq_map_break: {}
...
\cs_new:Npn \__ztoc_tocline_index_bynametitle_aux:nnn #1#2#3
  {
    \bool_lazy_all:nTF
      {
        { \exp_args:Ne \str_if_eq_p:nn
...
\cs_generate_variant:Nn \ztoc_tocline_index_bynametitle:nnN
  { nnc, eec, eeN }

% get class level by name & title(expandable)
\cs_new:Npn \ztoc_tocline_level_bynametitle:Nnnn #1#2#3#4
  {% #1:ori seq; #2:name; #3:title; #4:offset(int)
    \int_compare:nNnT
      { \ztoc_tocline_index_bynametitle:nnN {#2}{#3}#1 }
        < 
      { 0 }{ \relax } % TODO: handle non-exist case
    \ztoc_get_class_level_expandable:e
      {
        \prop_item:en
          {
...
\NewExpandableDocumentCommand{\ztocindex}{sO{toc}mm}
  {
    \IfBooleanTF{#1}
      {
...
\NewExpandableDocumentCommand{\ztoccard}{O{toc}mm}
  {
    \ztoc_tocline_card:nnc
      { #2 }{ #3 }
      { g_ztoc_keyval#1_seq }
  }
\NewExpandableDocumentCommand{\ztochilevel}{m}
  {
...
\prg_new_conditional:Npnn \ztoc_tocline_level_compare:Nnnnnn #1#2#3#4#5#6
  ...
  {% #1:ori seq; #2:name; #3:title; 
   % #4:index_1 offset; #5:index_2 offset; 
   % #6:operator
    \int_compare:nNnT 
      { \ztoc_tocline_index_bynametitle:nnN {#2}{#3}#1 }
        < 
    ...
    \int_compare:nTF
      {
        ( 
          \ztoc_tocline_level_bynametitle:Nnnn 
            #1 {#2}{#3}{#4} 
        )
        #6
        ( 
...
\prg_generate_conditional_variant:Nnn \ztoc_tocline_level_compare:Nnnnnn
  { c, Nee, cee }{ p, T, F, TF }
\prg_new_conditional:Npnn \ztoc_tocline_level_curnext_compare:Nnnn #1#2#3#4 
  { p, T, F, TF }
  {% #1:ori seq; #2:name; #3:title; #4:operator
    \ztoc_tocline_level_compare:NnnnnnTF #1 {#2}{#3}{0}{1}{#4}
      { \prg_return_true:  }
      { \prg_return_false: }
  }
% check if first/middle/last tocline(expandable)
\prg_new_conditional:Npnn \ztoc_if_first_tocline: {p, T, F, TF}
  {
    \int_compare:nNnTF 
      { \__ztoc_use_toc_group_int:e {\ztoc@current@class} }
      = { 1 }
...
\prg_new_conditional:Npnn \ztoc_if_middle_tocline:Nnn #1#2#3 
  { p, T, F, TF }
  {% #1:ori seq; #2:name; #3:title
    \ztoc_tocline_level_curnext_compare:NnnnTF #1 
      { #2 }{ #3 }{ = }
      { \prg_return_true:  }
      { \prg_return_false: }
  }
\prg_generate_conditional_variant:Nnn \ztoc_if_middle_tocline:Nnn
  { c, Nee, cee }{ p, T, F, TF }
\prg_new_conditional:Npnn \ztoc_if_last_tocline:Nnn #1#2#3 
  { p, T, F, TF }
  {% #1:ori seq; #2:name; #3:title
    \ztoc_tocline_level_curnext_compare:NnnnTF #1 
      { #2 }{ #3 }{ > }
...
\prg_generate_conditional_variant:Nnn \ztoc_if_last_tocline:Nnn
  { c, Nee, cee }{ p, T, F, TF }

% user interface of tocline position check:
\newcommand{\ztociffirst}[2]
  {
    \ztoc_if_first_tocline:TF 
      { #1 }{ #2 }
  }
\NewExpandableDocumentCommand{\ztocifmiddle}{O{toc}mm}
  {
    \ztoc_if_middle_tocline:ceeTF { g_ztoc_keyval#1_seq }
      { \ztocname }{ \ztoctitle }
      { #2 }{ #3 }
  }
\NewExpandableDocumentCommand{\ztociflast}{O{toc}mm}
  {
    \ztoc_if_last_tocline:ceeTF { g_ztoc_keyval#1_seq }
      { \ztocname }{ \ztoctitle }
      { #2 }{ #3 }
  }
...
%% document layer
% >> format toc data
\tl_gclear:N \ztocgroupparserOutput
\NewDocumentCommand{\ztocgroupparser}{O{toc}O{}mmmmm}
  {
    \exp_args:Nc \ztoc_group_parser:NNNnnnnn
      { g_ztoc_keyval#1_seq } 
      #3 \ztocgroupparserOutput
      {#4}{#5}{#6}{#7}{#2}
  }
...
\ztex_keys_define:nn { ztoc/option }
  {
    ...
    leader.content .code:n = { \def\ztoc@leader@content{#1} },
  }
...
\NewDocumentCommand{\zlocaltoc}{O{\jobname}mm}
  {
    ...
    \ztoc_table_typeset_before:
      ...
      \exp_args:Ne \clist_map_inline:nn { #3 }
        {
...
\DeclareDocumentCommand{\printglossaries}{}
  {
    \ztoc_table_typeset:Nn \g_ztoc_tog_seq 
      {}
  }
...
% marker form
\tl_new:N \g__zsect_marker_form_tl
\tl_gset:Nn \g__zsect_marker_form_tl 
  { \zsecname }
\cs_new:Npn \zsect_marker_form:n #1
  {
    \tl_gset:Nn \g__zsect_marker_form_tl
      { #1 }
  }
\cs_generate_variant:Nn \zsect_marker_form:n
  { o, e }
...
% user mark interface
\NewDocumentCommand{\zsectmarkform}{m}
  {
    \zsect_marker_form:n { #1 }
  }
\@onlypreamble\zsectmarkform
\NewDocumentCommand{\zsectmarkinsert}{mm}
  {
...
\ztex_new_templatetype:nn {sect}{4}
\ztex_declare_template_interface:nnnn {sect}{default}{4}
  {
    type            : tokenlist,
    hang            : boolean   = { true },
    break           : tokenlist,
    ...
    bookmark.cmd    : function{1} = #1,
  }
\ztex_declare_template_code:nnnnn {sect}{default}{4}
  {
    ...
    bookmark.cmd    = \__zsect_title_bookmark_cmd:n,
  }{
    % Instance args spec:
    %   #1: class name
    ...
    % 2. these groups aim to set TemplateKeys-related variables locally.
    % 3. '\refstepcounter' can NOT be inside a group, and 
    ...
    \group_begin:
      \AssignTemplateKeys
      \__zsec_public_macro_setup:nnnn
        { #1 }{ #2 }{ #3 }{ #4 }
      % start typesetting sec title:
      \exp_args:Nno \__zsect_mark_update:nn 
        { #1 }{ \g__zsect_marker_form_tl }
...
% setup sec title alias
\cs_new:Nn \__zsec_public_macro_setup:nnnn
  {
    ...
    \edef\zsect@tocnum {\cs:w the #1 \cs_end:}
    \edef\zsect@tocHnum
      {
        \ztexhyperTF
          { \cs:w theH#1 \cs_end: }
    ...
    % alias for users' interface:
    \cs_set_nopar:Npn \zsecclass   { #1 }
    \cs_set_nopar:Npn \zsectocname { #3 }
    \cs_set_nopar:Npn \zsecname    { #4 }
    \cs_set_nopar:Npn \zsecnum     { \zsect@num }
    \cs_set_nopar:Npn \zsectocnum  { \zsect@tocnum }
    \cs_set_nopar:Npn \zsectocHnum { \zsect@tocHnum }
  }
    ...
    \bool_if:NT \g__ztex_hyperref_bool 
      {
        \group_begin:
          ...
          \zsect_unexpand_protect:
            \xdef\zsect@temp@a
              {
                \__zsect_title_bookmark_cmd:n
                  {
                    \tl_if_empty:oTF {#2}{#3}{#2}
                  }
              }
          \zsect_restore_protect:
    ...
    \bool_if:NTF \l__zsect_title_inline_bool 
      { \hskip \l__zsect_title_spaf_skip\relax }
      {
        ...
        \bool_if:NTF \l__zsect_title_afterindent_bool
          { 
            \tl_gset:Nn \g__zsect_after_group_tl 
              {\@afterindenttrue }
          }{ 
            \tl_gset:Nn \g__zsect_after_group_tl 
              {\@afterindentfalse}
          }
        \tl_gput_right:Nn \g__zsect_after_group_tl 
          { \@afterheading }
      }
  }
...
% title body
\cs_new:Npn \__zsect_title_body:nnn #1#2#3
  {% #1:class; #2:bool; #3:name
    \group_begin: % this group make these formats act locally.
    ...
    \bool_lazy_all:nT
      {
        ...
        \bool_if:NT \l__zsect_title_hang_bool { \zsect_hang_from:n }
          {
      ...
      }
    {
      ...
      \zsec_normal_text:n { \l__zsect_title_after_tl }
      % NOTE: add this '\par' to make current paragraph be processed,
      %       thus, '\centering'-like commands will make sense. 
      \bool_if:NF \l__zsect_title_inline_bool {\par} 
    }
    \group_end:
  }
...
\cs_new:Npn \zsect_define_title:Nnn #1#2#3
  {% #1:cmd; #2:style; #3:keyval
    % setup instance
    \zcnt_safe_new:ee { \cs_to_str:N #1 }{}
    \exp_args:Ne \__zsect_setup_instance:nn 
      { \cs_to_str:N #1 @#2 }{ #3 }
    % TODO: check if instance valid:
    ...
    \DeclareDocumentCommand{ #1 }{sO{}m}
      {
        \IfBooleanTF{##1}
          {
            \exp_args:Neee \ztex_use_instance:nn {sect}
...
\cs_new_protected:Npn \__zsect_setup_instance:nn #1#2
  {
    \ztex_declare_instance:nnnn 
      { sect }{ #1 }
      { default }{ #2 }
    \ztex_declare_instance_copy:nnn 
      { sect }
      { #1-numberless }
      { #1 }
  }
    ...
    \clist_map_inline:nn { #3 }
      {
        \edef\zsect@tmp@instance@name
          { 
            \cs_to_str:N ##1 @#2
              \IfBooleanT{#1}{-numberless}
          }
        \IfInstanceExistsTF {ztexsect}
