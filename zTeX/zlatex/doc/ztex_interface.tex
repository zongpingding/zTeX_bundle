...
% \title{\texorpdfstring{\vspace{-4pt}z\TeX{} 接口文档}{zTeX 接口文档}} % for luatex
\title{\texorpdfstring{z\TeX{} 接口文档}{zTeX 接口文档}}   % for xetex
\author{Eureka}
\date{\today}
\begin{document}
\thmCommandRestore
...
\noindent{\sffamily\color{red}NOTE: \cls{ztex} 文档类目前已移除如下配置:}
\begin{DocExample}
...
\begin{keyval}[updated=2024-12-25, parent=ztex/toc]{column, title, title-vspace, stretch}
  ...
  因在 \pkg{l3keys} 中，当键为控制序列时，其不会自动展开; 故如果上述 \meta{toc} 的某一个子项内含有控制序列，那么务必通过
...
\begin{DocExample}[@@]
\ProvidesExplFile{ztex.module.@\meta{moduleA}@.tex}{2025/04/26}{1.0.0}{@\meta{discreption}@}
...
\begin{function}[added=2024-04-26]{\zfontsetfamily}
  ...
  \textbf{注意}: 目前此命令还未整合完成, 暂时不要使用此命令.
\end{function}
...
\begin{function}[added=2025-04-21]{\cref}
  ...
  \end{syntax}
  \zLaTeX{} 基于 \pkg{cleveref} 和 \pkg{zref-clever} 宏包提供 ``聪明引用'' 命令 \cs{cref}. 为统一命令, 
...
\vskip1.5em
\noindent{\color{red}\sffamily NOTE: 目前\pkg{cleveref} 宏包的维护情况不太明朗, 且和新版的 \TeX{}Live %
中的部分命令冲突, 这便是 \zLaTeX{} 同时提供二者的原因}
...
\vskip1.5em
\noindent\textcolor{red}{\sffamily NOTE:目前可以使用 \CusTeX{} 提供的接口进行水印的添加, 详细用法请参见其 \pkg{bgfg} 模块.}
...
\vskip1.5em
\noindent\textcolor{red}{\sffamily NOTE:目前可以使用 \CusTeX{} 提供的接口进行定制, 详细使用方法请参见其 \pkg{struct} 模块.}



\clearpage
\subsection{sclist 模块}
Semicolon list(简称为 \texttt{sclist}) 与 \pkg{expl3} 中的 ``\texttt{clist}'' 类似, 只不过其分隔符为 ``\texttt{;}'';
\ztex{} 创建此模块是为了更好的处理以 ``\texttt{;}'' 划分的数据, 主要是为了``\textbf{可展性}''; \ztex{} 的 \pkg{sclist}
库提供了以下的一些命令:


\begin{function}[added=2025-06-20, EXP]{\zcmd_sclist_patch:nn, \zcmd_sclist_patch:ne, \zcmd_sclist_patch:no}
  \begin{syntax}
...
\begin{DocExample}*
\ExplSyntaxOn
\def\clistA{\zcmd_sclist_patch:nn {\scan_stop:}{; a; 2; 3; ; }}
...
下面这个案例展示了如何使用 \pkg{sclist} 中的 \cs{sclist_map_tokens:nn} 和 \cs{sclist_map_tokens:Nn} 
两个命令:
\begin{DocExample}*
\ExplSyntaxOn
...
\clearpage
\subsection{cmd 模块}
\ztex{} 的 \pkg{cmd} 模块主要提供自定义命令接口, 其语法类似 Python 中的 \texttt{def(\meta{arg-spec}) \marg{code}};
该模块还提供了针对部分内核命令的 Patch, 比如 \pkg{clist} 模块. 该模块目前很不成熟, 请谨慎使用.
...
\begin{function}[added=2025-06-22]{
  ...
  \end{syntax}
  此命令为 \TeX{} 中 \cs{let} 这一原语的封装, 它的作用是局部的.
\end{function}
...
\begin{function}[added=2025-06-22]{
  ...
  \end{syntax}
  此命令为 \TeX{} 中 \cs{let} 和 \cs{global} 这两个原语的封装, 它的作用是全局的.
\end{function}



\clearpage
\subsubsection{clist patch}
...
\begin{function}[added=2025-06-20, EXP]{\zcmd_clist_patch:nn, \zcmd_clist_patch:ne, \zcmd_clist_patch:no}
  \begin{syntax}
...
\begin{DocExample}*
\ExplSyntaxOn
\def\clistA{\zcmd_clist_patch:nn {\scan_stop:}{, a, 2, 3, }}
\detokenize\expandafter{\expanded{\clistA}}
\ExplSyntaxOff
\end{DocExample}
...
\begin{function}[added=2025-06-25, pTF, rEXP]{\ztex_tl_if_eq:nn, \ztex_tl_if_eq:ne, \ztex_tl_if_eq:ee}
  \begin{syntax}
    \cs{ztex_tl_if_eq:nnTF} \Arg{tl-1}\Arg{tl-2}\Arg{true code}\Arg{false code}
  \end{syntax}
  ...
  当 \meta{tl-1} 与 \meta{tl-2} 中的 token 数量不一致时, \cs{ztex_tl_if_eq:nnTF} 会直接返回 \texttt{\Arg{false code}}, 
  比如 ``\verb|\ztex_tl_if_eq:nnTF {a{aa}}{aaa}| \verb|{true}{false}|'' 的返回结果为 ``\texttt{false}''.\par
  \vskip1em
...
\begin{function}[added=2025-06-25, pTF, rEXP]{\ztex_tl_if_in:nn, \ztex_tl_if_in:no, \ztex_tl_if_in:ne, \ztex_tl_if_in:ee}
  \begin{syntax}
    \cs{ztex_tl_if_in:nnTF} \Arg{tl-1}\Arg{tl-2}\Arg{true code}\Arg{false code}
  \end{syntax}
  此命令与 \textsf{l3tl} 中默认的 \cs{tl_if_in:nnTF} 含义、用法均相同(用于测试 \meta{tl-2} 能否在 \meta{tl-1} 中找到),
  ...
  \vskip1em
  \noindent{\color{red}\sffamily NOTE: 1. 在 \hologo{LuaTeX} 下, 此命令暂时没有发现任何的明显缺陷; \\ 
    \hphantom{NOTE:} 2. 目前该函数内部采用的字符串匹配算法比较低效, 后续也许会采用 KMP \\
      \hphantom{NOTE: 2.} 算法进行重写; \\
    \hphantom{NOTE:} 3. 此函数基于 \cs{int_step_tokens:nn}, 所以请确保你的 \textsf{l3kernel} 版本在 2025-\\ 
      \hphantom{NOTE: 2.} 01-15 之后.}
\end{function}
\begin{DocExample}*
\ExplSyntaxOn
...
\begin{function}[added=2025-06-21, rEXP, pTF]
  {
    \ztex_head_tail_if_eq:nnn,
    \ztex_head_tail_if_eq:enn,
    \ztex_head_tail_if_eq:eee,
  }
  \begin{syntax}
    \cmd{\ztex_head_tail_if_eq:nnnTF} \marg{tl}\marg{head}\marg{tail}
...
\begin{function}[added=2025-06-21, rEXP, pTF]{
  ...
  \begin{syntax}
    \cmd{\ztex_head_tail_if_eq:nnnTF} \marg{tl}\marg{index}\marg{token}
    ~~~~~~\Arg{true code}\Arg{false code}
  \end{syntax}
...
\begin{function}[added=2025-06-25, rEXP]
  {
  ...
  \begin{syntax}
    \cs{ztex_tl_replace_once:nnn} \meta{tl} \Arg{old tokens} \Arg{new tokens}
  \end{syntax}
...
\begin{function}[added=2025-06-25, rEXP]
  ...
  \begin{syntax}
    \cs{ztex_tl_replace_all:nnn} \meta{tl} \Arg{old tokens} \Arg{new tokens}
  \end{syntax}
...
\ExplSyntaxOff
Replace Once:\detokenize\expandafter{\expanded{\TTTa}}\par
Replace All :\detokenize\expandafter{\expanded{\TTTb}}
\end{DocExample}
...
\clearpage
\subsubsection{命令定义}
\begin{function}[added=2025-06-19]{\znewcmd, \zsetcmd, \zgsetcmd}
  \begin{syntax}
    \cs{znewcmd}\marg{arg-spec}\marg{code}
  \end{syntax}
  用户可以使用这三个命令创建控制序列, \meta{arg-spec} 的格式为: \texttt{\meta{var}:\meta{type}=\meta{default}};
...
\begin{function}[added=2025-06-19, EXP]{\cmdvar}
  \begin{syntax}
    \cs{zcmdvar}\marg{var}
  \end{syntax}
  此命令用于引用声明的变量, 被引用的变量一般含有数字, 横线, 下划线等特殊字符.
\end{function}
...
\begin{function}[added=2025-04-21]{\CUSLoadLibrary}
  ...
  出现加载 \CusTeX{} 的库后与 \zLaTeX{} 原本设置冲突的问题, 所以在 \zLaTeX{} 中应当谨慎使用 \CusTeX{} 的库.
\end{function}
...
本节主要介绍 \ztex{} 中提供的各类库(library), 这些 library 用于优化用户 \LaTeX{} 的文档书写和阅读体验。
部分 library 是对 \ztex{} 中原始功能的增强，但与此同时，文档的编译速度势必会稍微减慢，所以请酌情加载 \ztex{}
中的库.

\ztex{} 中所有的库均不会自动加载，用户需要使用 \cmd{\ztexloadlib}\texttt{\marg{library name}} 手动加载, 
...
下面这个案列展示了 \ztex{} 中 library 的加载方式:
\begin{DocExample}
% \documentclass{ztex}
\ztexloadlib{fancy}
\ztexloadlib{alias}
...
\begin{function}[updated=2025-04-25]{\zaliasOn, \zaliasOff}
  ...
  命令前缀, 默认为 ``OLD'';
  如果在此二命令之外使用 \pkg{alias} 库中的别名命令, 那么 \zLaTeX{} 会抛出错误.\par
  \textbf{注意}: 在正文中可以多次或嵌套使用此二命令, 但必须成对出现, 否则将会导致编组不匹配, 从而无法编译得到最终的文档.
\end{function}
\begin{DocExample}*[@@]
...
\begin{function}[updated=2025-04-25]{zalias}
  ...
  \meta{prefix} 用于设置当前文档中已存在的(外部)命令前缀,  默认为 ``OLD''; \par
  \textbf{注意}: 在正文中可以多次使用此环境, 且可以嵌套使用.
\end{function}
\begin{DocExample}*
\begin{zalias}
$\B{Q} \cong \B{Z}$
...
因为 \pkg{alias} 中与这一部分相关的命令依赖于 \cs{int_step_tokens:nn}, 而这个命令在 2025-01-15 之后
才正式被添加到 l3kernel 中.


\begin{function}[added=2025-06-20]{\mat, \pmat, \bmat, \Bmat, \vmat, \Vmat}
  \begin{syntax}
    \cs{mat}\{
      ~~\meta{item-11}, ..., \meta{item-1n};
      ~~~~...
      ~~\meta{item-m1}, ..., \meta{item-mn};
    \}
  \end{syntax}
  这系列命令用于输出排版矩阵, 其维度为 $m\times n$; ``\texttt{p}'' 的含义与 \pkg{amsmath} 宏包
  中 \verb|\pmatrix| 命令内的 ``\texttt{p}'' 含义相同, ``\texttt{b}, \texttt{v}'' 等参数的含义同理.
\end{function}
\begin{DocExample}*
\begin{align*}
...
\begin{function}[added=2025-06-20]{\imat, \admat}
  \begin{syntax}
    \cs{imat} ~\Arg{filler}\{\meta{item-1}, ..., \meta{item-n}\}
    \cs{admat} \Arg{filler}\{\meta{item-1}, ..., \meta{item-n}\}
  \end{syntax}
  此二命令用于生成对角矩阵或反对角矩阵, 其维度为 $n\times n$; \meta{filler} 用于指定
  非对角线元素, \meta{item} 中空值默认为 ``1''; \textbf{注意:} 此命令需结合上面的 \cs{mat}, \cs{pmat} 
  等命令使用.
\end{function}
\begin{DocExample}*
\begin{align*}
...
\begin{function}[added=2025-06-20]{\zmat}
  ...
  可选值有 ``\texttt{i, a, z}''. \textbf{注意:} 此命令不能单独使用, 用户需要将此命令置于一个矩阵环境中,
  或置于上面的 \cs{mat}, \cs{pmat} 等命令中.
\end{function}
\begin{DocExample}*
\begin{align*}
  \mat{\zmat{4}} = 
...
\begin{function}[added=2025-06-20]{\jmat, \hmat}
  \begin{syntax}
    \cs{jmat}\oarg{keyval}\{
    ~~\meta{dep-1},   ..., \meta{dep-m};
    ~~\meta{indep-2}, ..., \meta{indep-n}
    \}
    \cs{hmat}\oarg{keyval}\{
    ~~\meta{dep-1};
    ~~\meta{indep-1}, ..., \meta{inde-n}
    \}
  \end{syntax}
  此二命令分别用于输入 Jacobian 和 Hessian 矩阵, 前者是 $m\times n$ 的, 后者是 $1\times n$ 的; 
  \meta{keyval} 用于指定(矩阵的) 的排版样式; \meta{dep-i} 表示第 $i$ 个自变量, \meta{indep-i} 表示
  第 $i$ 个因变量.
\end{function}
...
\begin{keyval}[parent=ztex/zalias/jhmat]{b, c, s}
  ...
  \meta{c} 用于设置矩阵中每个公式的显示方式, 默认为 ``\cs{textstyle}'';
...
\begin{function}[added=2025-06-20]{\gmat}
  \begin{syntax}
    \cs{gmat} \{\meta{v-1}, ..., \meta{v-n}\}
  \end{syntax}
  此命令用于生成 Gram 矩阵, 其维度为 $n\times n$; 此命令仅为后续 \cs{xmat} 命令的一个特例. 
  \textbf{注意}:此命令仅返回矩阵对应的数据, 用户应将此命令置于一个合法的矩阵环境中.\par
  \vskip1.5em
  \noindent{\color{red}\sffamily NOTE: 请不要将此命令置于 \cs{mat}, \cs{pmat} 等命令中. }
\end{function}
...
\begin{function}[added=2025-06-20]{\xmat}
  ...
  \end{syntax}
  此命令用于自定义矩阵的生成方式, 其维度为 $m\times n$; 矩阵元素由 \cs{\meta{matcmd}} 指定, 
  \cs{\meta{matcmd}} 接受两个参数, 分别表示该元素的横坐标与纵坐标. \textbf{注意}:此命令仅返回
  矩阵对应的数据, 用户应将此命令置于一个合法的矩阵环境中; 同时也应确保 \cs{\meta{matcmd}} 是 Robust 的.
  \par \vskip1.5em
  \noindent{\color{red}\sffamily NOTE: 1. 此处的 \cs{xmat} 命令与 \pkg{pyhsics2} 宏包中的 \cs{xmat} 命令不同;\\
  \hphantom{NOTE:} 2. 请不要将此命令置于 \cs{mat}, \cs{pmat} 等命令中.}
\end{function}
\begin{DocExample}*
...
\begin{function}[added=2025-06-22]{
  ...
  被重定义为 Robust, 在此范围之外, 该命令将恢复为其原始定义.
\end{function}
...
\begin{function}[added=2025-06-22]{\z@mat@plain}
  \begin{syntax}
...
\cs{zslidetoc@labelset} 用于设置 slide 模式下目录条目的格式. \meta{extra width} 表示 \cs{thecontentslabel}
右侧额外的间距. \meta{item} 可以使用 \cs{thecontentslabel}, \cs{zslidesecIcon}, \cs{zslidesubsecIcon} 或
其它用户自定义符号.
...
\vskip2em
\noindent\textcolor{red}{\sffamily NOTE: 若用户在自定义定理类环境样式时需要更改 \ztex{} 的默认配色, 请将 \cs{ztex_keys_set:nn} 或
...
\begin{todolist}
  ...
  \item \pkg{alias} 库中与矩阵相关的 ``\cs{mat}, \cs{pmat}, ...'' 命令并没有很好的实现内容(数据)和(排版)格式的分离, 它们这几个命令应该
    仅用于矩阵的排版, 而非数据的生成.
  \item \pkg{alias} 库中矩阵相关的命令, 能否实现自动设置 \cs{arraystretch} 的值 ??
  \item 把原始的 \pkg{framed} 宏包替换为 \textsf{CUS} 宏集中的 \pkg{framedmulticol} 宏包.
  \item 修复 \cs{qedsymbol} 位置不正的问题, 或者参考 \pkg{amsthm} 宏包直接写一个新的 \cs{zqedhare} 命令.
