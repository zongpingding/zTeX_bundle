...
\begin{table}[H]
  \begin{tblr}{
    colspec={|X[1.25, c]|X[1.1, c]|X[1.65, c]|X[1.1, c]|X[1, c]|X[1.1, c]|X[2, c]|X[2, c]|},
...
\begin{enumerate}
  ...
  \item 封装 \texttt{\lastbox} 相关命令, 实现段落的分割和盒子的跨页需求.
  ...
  \item 在 \pkg{pageinfo} 中实现一个强化的 \cs{marginpar} 命令, 目的是成为 \pkg{sidenotes} 宏包的一个可选替代.
