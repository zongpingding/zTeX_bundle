...
\section{宏包机制}
\begin{function}[updated=2024-11-05]{zslide-title-page, zslide-last-page}
  \begin{syntax}
...
\begin{function}[updated=2024-11-05]{\zlatexSetup}
  ...
  \end{syntax}
  \zlatex{} 接受一系列的键值对进行配置，部分的配置仅可以在加载文档类时指定. 为区分二者，我们约定:
  \begin{itemize}
...
两个层级; 第一层中的 \zkey{layout, mathSpec, toc, packageOption, classOption, toc} 均具有自己的独立子键(\meta{sub-key}), 其余的键
...
\end{DocExample}
使用样例请参见命令 \cs{zlatexThmNoteEmptyTF} 中的示例.
...
\end{function}
\begin{DocExample}
\zlatexThmTocAdd[section]{name=SEC-NAME}
\end{DocExample}
...
\begin{function}[updated=2024-12-05]{\zlatexThmTocSymbol}
  ...
  \end{syntax}
  此命令用于分别设置所有定理类环境名在目录中的前缀, 仅能在导言区使用, 命令 \cs{zlatexThmTocSymbolClear} 可以用于清楚其设置的前缀. 
  所有可用的选项为:
\end{function}
\begin{DocExample}
\zlatexThmTocSymbol{
...
\vskip2em
\noindent\textcolor{red}{\sffamily WARNING: 此 library 加载的大量自定义命令可能会与已有的命令的冲突, 请用户%
在加载此 library 前做出仔细的考虑.}
...
\begin{function}[updated=2024-11-05]{zslide@titlepage, zslide@lastpage}
  \begin{syntax}
    \cs{pageref}{zslide@titlepage}
    \cs{pageref}{zslide@lastpage}
  \end{syntax}
  上述两 Targets 由命令 \cs{hyper@anchor} 设置, 分别应用于引用当前文档的第一页和最后一页, 在 zslide 中, 标题页的页码为 0.

  注意:普通用户不应该直接使用这两个 Targets，此二 Targets 主要提供给模板的开发者, 用于应使用位于首页和尾页的 \texttt{zslide-title-page} 
...
\begin{function}[updated=2024-11-05]{\zslideFrameCntAux:nn}
  ...
  \meta{number}; \meta{name} 一般为一大写罗马数字, 如 I, II, III, IV 等. 此命令在制作目录或向后搜集文档内容时
  是十分有用的.
\end{function}
...
\vskip2em
\textcolor{red}{\sffamily 如用户在自定义定理类环境样式时需要更改 \zlatex{} 的默认配色, 请将 \cs{zlatex_keys_set:nn} 命令放置于%
命令 \cs{zlatexThmStyleNew} 对应样式的 \meta{preamble} 中而非 \meta{option} 中, 否则 \zlatex{} 中的一系列与 \cs{zlatexColorSetup} 相关的函数将失去%
对新定义数学类环境样式的色彩控制能力.}
...
\ztool_resize_to_wd:nn {25pt} {Hello,~world!}
\ExplSyntaxOff
\end{DocExample}
