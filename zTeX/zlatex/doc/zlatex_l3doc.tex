...
\begin{function}[updated=2024-12-05]{ztool}
  ...
  \end{syntax}
  本宏集以独立实现了一个 \pkg{ztool} 宏包, 此模块中包含原来已被废弃的 \pkg{l3sys-shell} 中的所有命令.
...
\begin{function}[added=2025-01-15]{\zlatexHyperTF, \zlatexFancyTF, \zlatexMarginTF, \zlatexSlideTF, \zlatexFontConfigTF, \zlatexMathAliasTF, \zlatexBibIndexLoadTF}
  \begin{syntax}
...
zslide 中的坐标系统:如果不加说明，那么 zslide 中的坐标系统均以当前页面的左上角为原点，取向下向右为正方向. 这就意味着你的纵坐标
往往为负值，横坐标往往为正值.
...
\begin{function}[updated=2024-11-05]{\_zslide_status_bar:nnnn}
  ...
  \end{syntax}
  此命令用于创建 slide 的页面背景色快, 为方便叙述，我们称其为 \meta{BOX}. 其中 \meta{coordinate} 为此 \meta{BOX} 的左下角坐标, 形如
  \zlatexVerb{(10pt, -.1\paperwidth)}, 以当前页面的左上角为原点, 向下向右为正方向; \meta{type} 为状态栏类型, 目前所有可选值有:\cmd{UR}, \cmd{UL}, 
...
\begin{function}[updated=2024-11-05]{\_zslide_status_info:nnnn}
  ...
  左上角为原点, 向下向右为正方向.; \meta{width} 为当前 \meta{BOX} 的(弹性)宽度, 接受一个浮点数, 以 \cmd{\paperwidth} 为单位. \meta{content} 
...
\LaTeX{} 文档时启用 \cmd{-shell-escape} 参数, 否则此系列命令将不会指向任何操作.
...
\clearpage
\subsection{文件 IO 操作}
本部分主要介绍 \pkg{ztool} 中实现的文件 IO 操作, 包括: 读取文件, 写入文件, 追加文件等操作.
...
\begin{function}[updated=2024-12-05]{\ztool_read_file_as_seq:nnN, \ztool_read_file_as_seq:neN, \ztool_read_file_as_seq:nnc, \ztool_read_file_as_seq:nec}
  ...
  \end{syntax}
  此命令用于读取文件 \meta{file} 的内容, 并将其存放在 \meta{seq} 中, 如果 \meta{file} 不存在, 则会返回一个空的 \meta{seq}.
...
\begin{function}[updated=2024-01-05]{\ztool_append_to_file:nn, \ztool_append_to_file:no, \ztool_append_to_file:nf, \ztool_append_to_file:ee}
  \begin{syntax}
    \cs{ztool_append_to_file:nn} \marg{file}\marg{content}
  \end{syntax}
  此命令用于将 \meta{content} 追加到文件 \meta{file} 中, 如果 \meta{file} 不存在, 则会创建一个名为 \meta{file} 的新文件.
\end{function}


\begin{function}[updated=2024-01-05]{\ztool_replace_file_line:nnn, \ztool_replace_file_line:enn, \ztool_replace_file_line:ene, \ztool_replace_file_line:eee}
  \begin{syntax}
...
\begin{function}[updated=2024-01-05]{\ztool_insert_to_file:nnn, \ztool_insert_to_file:nen, \ztool_insert_to_file:nfn, \ztool_insert_to_file:een}
  \begin{syntax}
...
\begin{DocExample}*
\begin{filecontents}[noheader]{testIO.txt}
original content 
\end{filecontents}
\ExplSyntaxOn
...
\begin{function}[updated=2024-12-05]{\ztool_set_to_ht:Nn, \ztool_set_to_ht:Ne, \ztool_set_to_ht:Nc}
  \begin{syntax}
    \cs{ztool_set_to_ht:Nn} \meta{dim}\marg{content}
  \end{syntax}
  此命令用于将 \meta{content} 的高度保存在 \meta{dim} 这一寄存器中.
\end{function}

\begin{function}[updated=2024-12-05]{\ztool_set_to_wd:Nn, \ztool_set_to_wd:Ne, \ztool_set_to_wd:Nc}
  \begin{syntax}
    \cs{ztool_set_to_wd:Nn} \meta{dim}\marg{content}
  \end{syntax}
  此命令用于将 \meta{content} 的宽度保存在 \meta{dim} 这一寄存器中.
\end{function}

\begin{function}[updated=2024-12-05]{\ztool_gset_to_ht:Nn, \ztool_gset_to_ht:Ne, \ztool_gset_to_ht:Nc}
  \begin{syntax}
    \cs{ztool_gset_to_ht:Nn} \meta{dim}\marg{content}
  \end{syntax}
  此命令用于将 \meta{content} 的高度保存在 \meta{dim} 这一寄存器中, 并且此操作是全局的.
\end{function}

\begin{function}[updated=2024-12-05]{\ztool_gset_to_wd:Nn, \ztool_gset_to_wd:Ne, \ztool_gset_to_wd:Nc}
  \begin{syntax}
    \cs{ztool_gset_to_wd:Nn} \meta{dim}\marg{content}
  \end{syntax}
  此命令用于将 \meta{content} 的宽度保存在 \meta{dim} 这一寄存器中, 并且此操作是全局的.
\end{function}


\begin{function}[updated=2024-12-05]{\ztool_resize_to_wd:nn, \ztool_resize_to_wd:en, \ztool_resize_to_wd:ne}
  \begin{syntax}
    \cs{ztool_resize_to_wd:nn} \marg{dim}\marg{content}
  \end{syntax}
  此命令用于将 \meta{content} 的宽度调整为 \meta{dim}.
\end{function}

\begin{function}[updated=2024-12-05]{\ztool_resize_to_ht:nn, \ztool_resize_to_ht:en, \ztool_resize_to_ht:ne}
  \begin{syntax}
    \cs{ztool_resize_to_ht:nn} \marg{dim}\marg{content}
  \end{syntax}
...
\ztool_set_to_ht:Nn \l_ztool_tmp_H_dim {Hello,~world!}
\ztool_set_to_wd:Nn \l_ztool_tmp_W_dim {Hello,~world!}
\dim_use:N \l_ztool_tmp_H_dim \quad \dim_use:N \l_ztool_tmp_W_dim\\
% set content to dim
Hello,~world!\\
\ztool_resize_to_ht:nn {.5cm} {Hello,~world!}\\
\ztool_resize_to_wd:nn {15pt} {Hello,~world!}
\ExplSyntaxOff
\end{DocExample}
