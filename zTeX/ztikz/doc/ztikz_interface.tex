...
\vskip2em
\noindent{\sffamily\color{red} NOTE: 部分 Mathematica 中的函数 Mathics 也许并没有支持, 具体请参考 Mathics 的文档.}
...
\begin{function}[added=2025-05-15]{\ztikzHashCurrent}
  ...
  \end{syntax}
  此命令主要用于调试或与命令 \cs{ztikzForceToSkip} 配合使用; \texttt{\cs{ztikzHashCurrent}*} 将输出最近的一次 Hash 值计算结果;
...
\NewDocumentEnvironment{mermaid}{ m }
  {
    \xsim_file_write_start:ne {\c_true_bool}{\g__ztikz_mmd_path_tl/t@mp.mmd}
  }{
    ...
    \ztool_shell_mv:ee 
      ...
      {\t@mp@file.mmd}
    \ztikz_if_run_again:nenTF {\c_true_bool}{\t@mp@file.mmd}{#1}
      {
        \ztool_shell_escape:e
          {
            mmdc 
            \space-i\space \t@mp@file.mmd
            \space-o\space \t@mp@file.pdf 
            \space-f
          }
        \ztikz_term_info:e { Mermaid~running~on~file:'\t@mp@file.mmd'~... }
      }{
        \ztikz_term_info:e { Use~the~cached~Mermaid~result:'\t@mp@file.pdf'~... }
      }
...
\end{mermaid}
\begin{center}
  \includegraphics[width=.85\linewidth]{\mmdOutputFile}
\end{center}
\end{DocExample}
