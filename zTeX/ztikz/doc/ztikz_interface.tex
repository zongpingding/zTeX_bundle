...
\ztikzloadlib{cache, gnuplot, python, wolfram, l3draw}
...
把上述的源码保存为 \file{test.wls}, 然后在命令行运行:
\begin{DocExample}
...
\vskip1.5em
\noindent\textbf{注记}: \ztikz{} 的 \pkg{wolfram} 库可以在 Windwos/Linux/MacOs 三大平台上使用, 并且可以在
源码中键入 ``\verb|\, #, $, _, ^, &|'' 等特殊字符, 极大的弥补了 \href{https://github.com/stevenliuyi/latex-alpha2}{\pkg{latexalpha2}} 的不足.
...
上述的 \pkg{basic} 库将自动加载, 其他库需要用户手动加载. \pkg{basic} 库中仅包含了用于绘制点, 直线, 坐标轴和基本多边形等系列命令.
...
\begin{function}[added=2025-05-15]{\Plot}
  ...
  默认为 \texttt{-5:5}.\par
  \vskip1em
  \noindent{\sffamily\color{red} NOTE: 只需将 \meta{opacity} 设置为 0, 即可实现散点图绘制.}
\end{function}
...
\begin{function}[added=2025-05-15]{\ztikzHashCurrent}
  ...
  \end{syntax}
  此命令主要用于 debug, 用于输出截至目前位置所有缓存的 Hash 值, 以 \meta{separator} 分隔输出到 PDF.
...
\begin{function}[added=2025-05-15]{\zplot}
  ...
  \vskip2em
  \noindent{\sffamily\color{red}NOTE: 目前 \cs{zplot} 命令有 Bug, 用户暂时不应该使用.}  
\end{function}
