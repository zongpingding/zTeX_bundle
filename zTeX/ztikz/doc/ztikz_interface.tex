...
% \begin{comment}
\section{基本介绍}
直到今天, 其实已经有很多基于 \pkg{tikz} 开发的绘图宏包了, 它们有着不同的用途, 在不同的领域中你都能看到 \TikZ{} 的痕迹. 
...
该仓库中包含本宏集的源码, 用户手册以及一些测试用例; 当前宏集的稳定版本于 2025年 09 月发布, 
最新版本请切换到 ``dev'' 分支; 本手册适用于当前最新的 ``dev'' 分支.


\subsection{功能概述}
\zTikZ{} 宏包主要用于绘图与计算, 支持调用外部程序, 比如 Python, Mathematica, gnuplot; 同时也提供了调用缓存机制; 
虽然 \ztikz{} 提供了这些软件的调用接口, 但这并不意味着你需要安装以上的所有软件; \ztikz{} 中每一个软件的调用接口都是
互相独立的, 用户仅需在操作系统上安装自己需要功能对应的软件即可. \zTikZ{} 的功能概述如下: 
\begin{itemize}
...
\begin{DocExample}[++]
sudo ln -sf "/mnt/c/Program Files/Wolfram Research/WolframScript/wolframscript.exe" /usr/bin/wolframscript
\end{DocExample}
...
创建软连接的命令如下:
\begin{DocExample}[++]
sudo ln -sf "/mnt/c/Users/+\meta{name}+/AppData/Local/Programs/Python/Python312/Scripts/mathics.exe" /usr/bin/mathics
\end{DocExample}
\resetExampleUR
...
\vskip2em
\noindent{\sffamily\color{red} NOTE: 部分 Mathematica 中的函数 Mathics 也许并没有支持, 还请参考 Mathics 文档.}
...
\clearpage
\section{杂项}
\begin{function}[added=2025-09-22]{\zTikZ, \ztikz}
  它们是 \pkg{ztikz} 宏包的 logo. 除此之外, \pkg{ztikz} 还提供了 \pkg{hologo}
  相关的宏. 
\end{function}
\begin{DocExample}*
% \usepackage{hologo}
...
\def\exampleUR{}
\begin{DocExample}
  \draw/\ttfamily\oarg{key-value} plot [ycomb, \meta{other style}] file \marg{data};/
\end{DocExample}
\resetExampleUR
...
\def\exampleUR{}
\begin{DocExample}
  \draw/\ttfamily\oarg{key-value} plot\oarg{id} function\marg{function};/
\end{DocExample}
...
两者的调用格式如下：
\begin{DocExample}
  \draw/\ttfamily\oarg{key-value} plot [parametric,  \meta{id}]\marg{function};/
  \draw/\ttfamily\oarg{key-value} plot [raw gnuplot, \meta{id}]\marg{gnuplot code};/
\end{DocExample}
\resetExampleUR
...
\begin{keyval}[parent=ztikz/2dplot]{domain, style, marker}
  \begin{syntax}
    domain  = \meta{浮点数:浮点数; 浮点数:浮点数}>\dval{\meta{不定}}
...
\begin{function}[added=2025-05-15]{\ztikzHashCurrent}
  ...
  \end{syntax}
  此命令主要用于调试缓存相关的代码, 它常常与命令 \cs{ztikzForceToSkip} 配合使用;
  \texttt{\cs{ztikzHashCurrent}*} 将输出最近的一次 Hash 值计算结果;
  \cs{ztikzHashCurrent}\texttt{\oarg{separator}} 用于输出截至目前位置所有缓存的 Hash 值, 
  以 \meta{separator} 分隔输出到 PDF; \meta{separator} 默认为 ``\texttt{,}''.
\end{function}


\begin{function}[added=2025-05-29, EXP]{\ztikzCachedHash}
  \begin{syntax}
    \cs{ztikzCachedHash}\oarg{keyval}
  \end{syntax}
  此命令用于输出当前已缓存的 Hash 值, 应用场景较之 \cs{ztikzHashCurrent} 命令更加的广泛.
\end{function}


\begin{keyval}[parent=ztikz/cache/hash]{index, file, label}
  \begin{syntax}
    index = \meta{整数}>\dval{-1}
    file  = \meta{文件名}>\dval*{ztikz_output/ztikz.hash}
    label = \meta{字符串}>\dval*{\cs{g__zcache_latest_cache_label_tl}}
  \end{syntax}
  \meta{label} 默认情况下为当前最新的缓存标签; \meta{file} 为 Hash 值对应的缓存文件;
  \meta{index} 用于指定该 \meta{label} 所缓存的 Hash 值的索引, 默认为 -1, 即最新的 Hash 值;
\end{keyval}


\begin{function}[added=2025-05-15]{\ztikzForceToSkip}
  此命令会强制跳过(重新)运行它之后的第一个具有 cache 机制的环境或命令, 即使该环境或命令对应的 Hash 已经改变; 
  后续的 \cs{wolframResult} 或 \cs{wolframOuputFile} 命令对应的引用结果都将受到此命令的影响. \par 
  \textbf{注意}:当应用此命令后, 新产生的 Hash 值并不会被缓存; 该命令目前仅对 \pkg{python} 和 \pkg{wolfram}
...
\begin{function}[added=2025-05-21]{\ztikzForceToRun}
  ...
  \textbf{注意}:该命令目前仅对 \pkg{python} 和 \pkg{wolfram} 库中的命令和环境有效; 即使是该命令后续的
...
\clearpage
\subsubsection{编程接口}
\pkg{ztikz} 的 \pkg{cache} 库提供了一系列的编程接口, 用户可以利用这一系列的接口来编写外部程序
调用相关的命令或环境. 这系列的新建命令或环境将支持缓存机制, 目前 \pkg{cache} 库中提供的编程接口
和部分其它相关命令如下:
...
\begin{function}[added=2025-05-30]{\g_ztikz_file_hash_seq}
  ...
  对应的 Hash 值.\par 
  \textbf{注记}: 该命令由后续的 \cs{__zcache_hash_extract_all:nN} 命令设置得到.
\end{function}


\begin{function}[added=2025-05-30]{\g_ztikz_hash_nochg_run_bool, \g_ztikz_hashchg_norun_bool}
  这两个 bool 值用于控制 \cs{ztikz_if_run_again:nnnTF} 命令的行为, 前者为 ``\texttt{true}'' 时:
  \cs{ztikz_if_run_again:nnnTF} 命令的 Hash 校验会被强制跳过, 从而直接运行 \meta{true code}; 后者为 ``\texttt{true}'' 时:
  \cs{ztikz_if_run_again:nnnTF} 命令的 Hash 校验会被强制跳过, 从而直接运行 \meta{false code}. \par
  \textbf{注意}: 这两个 bool 值默认均为 ``\texttt{false}''; {\sffamily\color{red} 二者同时为
  ``\texttt{true}'' 会报错}. 
\end{function}
...
\begin{function}[added=2025-05-30]{\xsim_file_write_start:nn, \xsim_file_write_start:ne}
  \begin{syntax}
    \cmd{\xsim_file_write_start:nn} \marg{bool}\marg{file}
  \end{syntax}
  此命令来自 \pkg{xsimverb} 宏包, 用于将环境内容抄录到 \meta{file} 文件中; \meta{bool} 为布尔值, 如果该抄录环境需要接受
  参数，那么请将 \meta{bool} 置为 ``\texttt{\cs{c_true_bool}}'', 否则请置为 ``\texttt{\cs{c_false_bool}}''.\par 
  \textbf{注意}: 该命令需配合 \cs{xsim_file_write_stop:} 命令使用, 否则会报错.
\end{function}


\begin{function}[added=2025-05-30]{\xsim_file_write_stop:}
  该命令用于结束环境内容抄录, 需配合 \cs{xsim_file_write_start:nn} 命令使用.
\end{function}
...
\begin{function}[added=2025-05-30, TF]{
  ...
  \begin{syntax}
    \cmd{\ztikz_if_run_again:nnnTF}
    ~~~~\marg{bool}\marg{file/hash}\marg{label}
    ~~~~\marg{true code}\marg{false code}
  \end{syntax}
  \meta{bool} 用于控制第二个参数 \meta{file/hash} 的类型, \meta{bool} 为 ``\texttt{\cs{c_true_bool}}'' 时,
  \meta{file/hash} 需传入文件名, 否则应传入一个 Hash 值; \meta{label} 为该 Hash 值或文件 Hash 值对应的缓存标签;
  当该环境的 Hash 值不存在, Hash 值改变抑或该环境被置于命令 \cs{ztikzForceToRun} 后时, 将会运行 \meta{true code};
  当该环境的 Hash 值已存在或该环境被置于命令 \cs{ztikzForceToSkip} 后时, 将会运行 \meta{false code}.
\end{function}


\begin{function}[added=2025-05-31]{\ztikz_term_info:n, \ztikz_term_info:e}
  \begin{syntax}
    \cs{ztikz_term_info:n} \marg{message}
  \end{syntax}
  此命令与 \TeX{} 中的 \cs{typeout} 命令类似, 用于向终端输出信息 \meta{message}.
\end{function}
...
\clearpage
\subsubsection{私有接口}
本小节将介绍 \pkg{cache} 库中的私有命令和变量, 这部分命令或变量在用户定义一些命令或环境时可能是必须的; 
...
\begin{function}[added=2025-05-30]{\__zcache_hash_add:nn}
  \begin{syntax}
    \cs{__zcache_hash_add:nn} \marg{label}\marg{hash}
  \end{syntax}
  此命令用于将 \meta{hash} 添加到 \meta{label} 对应的缓存标签中.
\end{function}


\begin{function}[added=2025-05-30]{\__zcache_hash_extract_all:nN}
  \begin{syntax}
    \cs{__zcache_hash_extract_all:nN} \marg{file}\meta{seq}
  \end{syntax}
  该命令用于将文件 \meta{file} 中的所有 Hash 值提取后保存到到变量 \meta{seq} 中,
  \meta{seq} 中的每一项都对应一个 Hash 值.
\end{function}


\begin{function}[added=2025-05-30]{\__zcache_hash_extract_by_label:nnn}
  \begin{syntax}
    \cs{__zcache_hash_extract_by_label:nnn} \marg{file}\marg{label}\marg{index}
  \end{syntax}
  该命令会从文件 \meta{file} 中提取标签为 \meta{label}, 索引为 \meta{index} 的 Hash 值.
  该命令无返回值, 它会将 \cs{l__ztikz_current_hash_tl} 置为其提取的 Hash 值. 
\end{function}


\newgeometry{hmargin=2.5cm, vmargin=1.5in}
\subsubsection{编程接口案例}
...
\NewDocumentEnvironment{mermaid}{ m }
  {
    \exp_args:Ne \xsim_file_write_start:nn {\c_true_bool}{\g__ztikz_mmd_path_tl/t@mp.mmd}
  }{
    \xsim_file_write_stop:
    \ztikz_if_run_again:nenTF {\c_true_bool}{t@mp.mmd}{#1}
      {
        ...
        \ztikz_term_info:e { Mermaid~running~on~file:'\l__ztikz_current_hash_tl.mmd'~... }
      }{
        \ztikz_term_info:e { Use~the~cached~Mermaid~result:'\l__ztikz_current_hash_tl.pdf'~... }
      }
    ...
    \ztool_shell_mv:ee
      ...
      {\t@mp@file.mmd}
    \ztool_shell_mv:ee
      {\g__ztikz_mmd_path_tl/t@mp.pdf}
      {\t@mp@file.pdf}
    \xdef\mmdOutputFile{\t@mp@file.pdf}
  }
...
\begin{function}[added=2025-05-29]{\sympy}
  \begin{syntax}
    \cs{sympy}\marg{label}\marg{expression}
  \end{syntax}
  此命令主要用于调用 Python 的 \pkg{sympy} 库进行符号计算, \zTikZ{} 对此命令提供了 cache 机制; \meta{label} 中不能包含 ``:'', 其用于指定
  该命令的缓存标签, 该 \meta{label} 在当前文档中必须是唯一的; \meta{expression} 为符号表达式.  \pkg{python} 库中预
  定义了一系列的符号变量, 包括: \texttt{x, y, z, u, v, t}, 这些预定义变量无需用户再次声明.\par 
  \textbf{注意}: 默认的情况下, 此命令的返回结果中可能包含: ``\verb|^, _|'' 等数学环境中才能使用的字符, 故用户应尽量
  将此命令置于数学环境中.
\end{function}
\begin{DocExample}*
\[
...
\begin{function}[updated=2025-05-29]{pyfig}
  \begin{syntax}
    \cs{begin}\zarg{pyfig}\marg{label}\marg{output file} 
    ~~~~\meta{plot code} 
    \cs{end}\zarg{pyfig}
  \end{syntax}
  此环境用于调用 Python 进行绘图, 不会返回任何结果, 该环境具有缓存机制; \meta{label} 中不能包含 ``:'', 其用于指定该环境的缓存标签, 
  该 \meta{label} 在当前文档中必须是唯一的; \meta{output file} 用于指定代码 \meta{plot code} 的输出文件名, 
  \meta{output file} 中无需给出输出文件路径, 但需指定输出文件的拓展名;\par 
  \textbf{注意}: 针对不同的 \env{pyfig} 环境建议使用不同的 \meta{output file} 值; 用户不需要在代码末尾添加 \verb|plt.savefig()| 命令,
  \ztikz{} 会自动处理此问题. 代码在抄录过程中会保留用户的缩进格式, 从行首开始抄录, 所以请不要添加多余的行首缩进;
  请确保 \meta{output file} 与后续环境代码中的输出文件名保持一致, 否则会报错.
\end{function}


\begin{function}[added=2025-04-21, EXP]{\pyfigOutputFile}
  此命令将返回 \env{pyfig} 环境运行输出的文件名, 用户可以使用 \cs{input} 
  或 \cs{includegraphics} 之类的命令导入该文件.
\end{function}
...
\begin{function}[added=2025-05-21]{pycode}
  \begin{syntax}
  \cs{begin}\zarg{pycode}\marg{label}\marg{output file} 
  ~~~~\meta{any python code} 
  \cs{end}\zarg{pycode}
  \end{syntax}
  此环境用于调用 Python 执行环境中的 \meta{any python code}, 不会返回任何结果, 该环境具有缓存机制; \meta{label} 中不能包含 ``:'', 其用于指定该环境
  的缓存标签, 该 \meta{label} 在当前文档中必须是唯一的; \meta{output file} 用于指定该环境代码的输出文件名, \meta{output file} 中无需给出输出文件路径, 但需指定输出文件的拓展名; 
  随后 \ztikz{} 会自动调用 Python 执行该文件，该环境的运行结果保存于文件 \cs{pycodeOutputFile} 中, 用户后续仅需
  导入该文件即可;\par 
  \textbf{注意}: 针对不同的 \env{pycode} 环境建议使用不同的 \meta{output file} 值; 代码在抄录过程中会保留用户的缩进格式, 
  从行首开始抄录, 所以不要过度使用缩进; 请确保 \meta{output file} 与后续环境代码中的输出文件名保持一致, 否则会报错.
\end{function}


\begin{function}[added=2025-04-21, EXP]{\pycodeOutputFile}
  此命令将返回 \env{pycode} 环境运行输出的文件名, 用户可以使用 \cs{input} 
  或 \cs{includegraphics} 之类的命令导入该文件.
\end{function}


下面是一个关于 \env{pycode} 环境的简单使用示例, \file{table.py.txt} 对应的文件内容请参见 \cref{imple:code-data}.
\begin{DocExample}*[@@]
\input{./table.py.txt}
\begin{center}
...
\ztikz{} 的 \pkg{wolfram} 库可看作是原始宏包 \pkg{latexalpha2} 的一个新实现, 可以弥补 \href{https://github.com/stevenliuyi/latex-alpha2}{\pkg{latexalpha2}} 
宏包的一系列不足. 目前 \pkg{wolfram} 库已经实现 \pkg{latexalpha2} 中除 \cs{wolframanimation} 命令外的所有命令, 并且在兼容性, 易用性和可拓展性上
相较于原始的 \pkg{latexalpha2} 宏包都有了极大的提升. 例如, \ztikz{} 的 \pkg{wolfram} 库可以在 Windwos/Linux/MacOs 三大平台上使用; 
\pkg{wolfram} 库的环境源码中支持直接键入 ``\verb|\, #, $, _, ^, &|'' 等特殊字符. 下面是使用 \pkg{wolfram} 库时的一些注意事项:
\begin{itemize}
  \item 用户需注意 WolframScript 脚本中注释的写法, 不是 ``\verb|(* something*)|'', 而是 ``\verb|(* something *)|'',
    即注释内容不能够紧挨 ``\verb|*|'', 否则可能会造成 WolframScript 的解析错误.
  \item 由于 WolframScript 的限制, 脚本的后缀只能为: ``\texttt{.wls}'', 否则 WolframScript
...
\begin{function}[added=2025-05-15]{\wolframResult}
  ...
  仅引用部分计算结果, \meta{index} 为整数或整数表达式, 默认为 1.\par
  \vskip1em
  \noindent{\sffamily\color{red}NOTE: 因为 \cs{wolframResult} 会根据 ``$@_{12}$'' 去划分结果, 所以请确保 ``@''
  的 catcode 为 12, 否则可能会导致结果解析错误.}
\end{function}
...
\begin{function}[added=2025-05-29]{\wolfram}
  \begin{syntax}
    \cs{wolfram}\marg{label}\marg{code}
    \cs{wolfram}*\marg{label}\marg{code}
  \end{syntax}
  此命令用于调用 WolframScript 中的进行计算, 具有缓存机制; \meta{label} 中不能包含 ``:'', 其用于指定该命令的缓存标签, 
  该 \meta{label} 在当前文档中必须是唯一的; \meta{code} 为合法的 WolframScript 代码;
...
\begin{function}[added=2025-05-29]{\wolframTex}
  \begin{syntax}
    \cs{wolframTex}\marg{label}\marg{Tex code}
  \end{syntax}
  此命令和上述的 \cs{wolfram} 命令类似, 不同的是, 此命令会将 \meta{Tex code} 中的所有内容转化为对应的
  Mathematica/Mathics 代码, 返回的结果为 \LaTeX{} 代码. \meta{label} 中不能包含 ``:'', 其用于指定该命令的缓存标签, 
  该 \meta{label} 在当前文档中必须是唯一的; \par
  \vskip1em
  \noindent{\sffamily\color{red}NOTE: 由于此命令的实现原理较为复杂与特殊, 所以 \meta{Tex code} 
  和 \meta{label} 中均不能包含 ``\verb|$|'' 符号, 否则会出现解析错误.} 
\end{function}
\begin{DocExample}*
...
\begin{function}[added=2025-05-29]{\wolframTable}
  \begin{syntax}
    \cs{wolframTable}\marg{label}\marg{code}
    \cs{wolframTable}*\marg{label}\oarg{key-value}\marg{code}
  \end{syntax}
  此命令用于调用 Wolfram 引擎生成表格, 具有缓存机制; \meta{label} 中不能包含 ``:'', 其用于指定该命令的缓存标签, 
  该 \meta{label} 在当前文档中必须是唯一的; 前者(不带有 ``\texttt{*}'' 的命令) 不会在 PDF 
  中输出对应的表格, 此命令设置了 \cs{wolframTablePData}, \cs{wolframTableFData} 两个临时变量, 其中保存了表格数据; 
  后者(带有 ``\texttt{*}'' 的命令) 会在 PDF 中输出对应的表格, 
  表格的样式可以通过 \meta{key-value} 进行指定; \meta{code} 为合法的 WolframScript/Mathics 代码;
\end{function}


\begin{keyval}[parent=ztikz/wolfram/table]{format, header, hdbt-rule, cell-cmd}
  \begin{syntax}
    format    = \meta{列格式}>\dval{*\{12\}\{l\}}
    header    = \meta{表头}>\dval{空}
    hdbt-rule = \meta{\textbf{false}|true}>\dval{false}
    cell-cmd  = \meta{单元格命令}>\dval{\#1}
  \end{syntax}
  \meta{format} 用于设置表格的列格式; \meta{header} 用于设置表格的表头, 该参数会在表格的第一行输出;
  \meta{hdbt-rule}(header bottom rule) 用于设置是否显示表头与后续表格内容之间的横线, 默认不显示; 
  该选项仅对 ``\cs{wolframTable}\texttt{*}'' 命令有效, 即仅在需要排版表格时有效; 
  \meta{cell-cmd} 接受一个参数, 其将应用于每一个 \cs{wolframTablePData} 中的单元格(不包括表头), 
  使用 ``\verb|#1|'' 表示当前单元格内容;
\end{keyval}


\begin{function}[added=2025-05-18, EXP]{\wolframTablePData, \wolframTableFData}
  这两个命令表示最近一次 \cs{wolframTable} 命令的运算结果, 不含有 \cs{hline}, \cs{midrule} 等命令, 即二者仅含有纯表格数据; 
  \cs{wolframTablePData} (Part Data)返回的数据不包括表格的表头, \cs{wolframTableFData} (Full Data)返回的数据包括表格的表头; 
  此二命令可以传入 \pkg{tabularray} 等表格排版宏包的数据输入. \textbf{注意}: 命令 \cs{wolframTable} 中的 \meta{cell-cmd} 
  不建议与 \pkg{tabularray} 中的 \meta{cells/cmd} 混用.
\end{function}
\begin{DocExample}*
...
\begin{function}[added=2025-05-29]{\wolframSolve}
  \begin{syntax}
    \cs{wolframSolve}\marg{label}\oarg{key-value}\marg{equation}
    \cs{wolframSolve}*\marg{label}\marg{full code}
  \end{syntax}
  此命令用于调用 WolframScript 中的进行方程的求解, 具有缓存机制; \meta{label} 中不能包含 ``:'', 其用于指定该命令的缓存标签, 
  该 \meta{label} 在当前文档中必须是唯一的; \meta{equation} 为对应方程的表达式;
...
\begin{function}[added=2025-05-29]{\wolframDSolve}
  \begin{syntax}
    \cs{wolframDSolve}\marg{label}\oarg{key-value}\marg{equation}
    \cs{wolframDSolve}*\marg{label}\marg{full code}
  \end{syntax}
  此命令用于调用 WolframScript 中的进行微分方程的求解, 具有缓存机制; \meta{label} 中不能包含 ``:'', 其用于指定该命令的缓存标签, 
  该 \meta{label} 在当前文档中必须是唯一的; \meta{equation} 表示方程的表达式;
...
\begin{function}[added=2025-05-29]{wolframGraphics}
  \begin{syntax}
    \cs{begin}\zarg{wolframGraphics}\marg{label}\oarg{spec} 
    ~~~~~\meta{plot code}
    \cs{end}\zarg{wolframGraphics}
  \end{syntax}
  此环境用于调用 WolframScript 进行绘图, 具有缓存机制; \meta{label} 中不能包含 ``:'', 其用于指定该环境的缓存标签, 
  该 \meta{label} 在当前文档中必须是唯一的; \meta{spec} 用于设置图片的的排版参数, 默认为空,
  此时该环境不会返回任何的结果, 可以通过 \cs{wolframOuputFile} 调用其产生的文件; 
  \meta{spec} 可以设置值, 对应图片的排版参数, 比如 \verb|width=10em|; 若 \meta{spec} 非空, 则该环境的
  返回值为: \texttt{\cs{includegraphics}\oarg{spec}\zarg{\meta{path}/\meta{HASH}.pdf}}, 其中 \meta{HASH} 
...
\begin{function}[added=2025-05-15]{\zscolor, \zfcolor}
  \begin{syntax}
    \cs{zscolor}\marg{l3color}
    \cs{zfcolor}\marg{l3color}
  \end{syntax}
  \meta{l3color} 为 \pkg{l3draw} 中合法的颜色表达式; \ztikz{} 对常见的颜色预定义了其对应
  的 ``l3color'' 变量, 用户可以直接使用这部分颜色.
\end{function}


\begin{function}[added=2025-05-31]{\zfevenodd, \zfnozero}
  命令 \cs{zfevenodd} 用于指定区域内外分割规则为 -- ``奇偶规则'';
  命令 \cs{zfnozero} 用于指定区域内外分割规则为 -- ``非零规则'', 
\end{function}
...
\begin{function}[added=2025-05-15]{\zshift, \zxscale, \zyscale, \ztrans}
  \begin{syntax}
    ...
    \cs{ztrans}\marg{a}\marg{b}\marg{c}\marg{d}
  \end{syntax}
  这一系列的命令用于对坐标轴进行仿射变换, \cs{ztrans}\verb|{a}{b}{c}{d}| 对应的仿射变换矩阵为:
  \[\begin{bmatrix}
    a & c\\
    b & d
  \end{bmatrix}\]
...
\begin{function}[added=2025-05-15]{\zusepath}
  \begin{syntax}
    \cs{zusepath}\oarg{style}
  \end{syntax}
  此命令用于显示最终的路径, \meta{style} 默认为 ``\texttt{draw}'', 其余的可选值
...
\noindent\textbf{备注}: \(\text{faddeeva}(z)\): rescaled complex error function



\clearpage
\subsection{marker 样式}
...
\begin{todolist}
  ...
  \item *[参考 \pkg{cache} 库一节中 ``\env{mermaid}'' 环境的实现]实现 \env{wolframAny} 环境, 该环境实现的功能类似 \env{pycode}.
  \item [2025-05-29]重写缓存机制对应的函数 \cs{ztikz_hash_if_change:nn}, 目前不够灵活(或许直接使用 \href{https://github.com/leo-colisson/robust-externalize}{\pkg{robust-externalize}} 宏包).
  \item [2025-05-29]针对 \pkg{cache} 库, 需要清除多余的 Hash 值: 例如某个环境/命令产生的原 Hash 值为 ``A'', 对应环境/命令中的参数改变后，其 Hash 值变为了 ``B'', 那么此时需要清除原始的 ``A''.
  \item [2025-05-29]实现 \cs{ztikzForceToSkip} 命令: 如何处理 \cs{xsim_file_write_start:nn} 环境 ? 如何处理多个 \cs{ztikzForceToRun} ? 也许使用 \cs{str_mdfive_hash:n} 会有帮助 ?
\end{todolist}
...
% ----------------------------------------------------------------------
\tikzexternaldisable
\newgeometry{left=.5in, top=0pt, right=.35in, bottom=0pt}
\ztexDocPrintSource
