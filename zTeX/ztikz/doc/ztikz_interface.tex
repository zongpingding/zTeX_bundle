...
\begin{function}[added=2025-05-15]{\ztikzForceToSkip}
  ...
  \textbf{注意}: 目前该命令目前很不成熟, 仅对 \pkg{python} 库中的 \cs{sympy} 命令有效; 尽量避免连续使用
  多个 \cs{ztikzForceToSkip} 命令, 否则会导致引用的缓存结果出现较大的偏差.
\end{function}
...
\begin{function}[updated=2025-05-21]{pyfig}
  \begin{syntax}
    \cs{begin}\zarg{pyfig}\marg{output file} 
    ~~~~\meta{plot code} 
    \cs{end}\zarg{pyfig}
  \end{syntax}
  此环境用于调用 Python 进行绘图, 不会返回任何结果, 该环境具有缓存机制;
  \meta{output file} 用于指定代码 \meta{plot code} 的输出文件名, \meta{output file} 中无需给出输出文件路径, 但需指定输
  出文件的拓展名;\par 
...
\begin{function}[added=2025-05-21]{pycode}
  \begin{syntax}
  \cs{begin}\zarg{pycode}\marg{output file} 
  ~~~~\meta{any python code} 
  \cs{end}\zarg{pycode}
  \end{syntax}
  此环境用于调用 Python 执行环境中的 \meta{any python code}, 不会返回任何结果, 该环境具有缓存机制; 
  \meta{output file} 用于指定该环境代码的输出文件名, \meta{output file} 中无需给出输出文件路径, 但需指定输出文件的拓展名; 
...
\begin{function}[added=2025-05-18]{\wolframTable}
  \begin{syntax}
    \cs{wolframTable}\marg{code}
    \cs{wolframTable}*\oarg{key-value}\marg{code}
  \end{syntax}
...
\begin{todolist}
  ...
  \item 实现 \cs{ztikzForceToSkip} 命令: 如何处理 \cs{xsim_file_write_start:nn} 环境 ? 如何处理多个 \cs{ztikzForceToRun} ? 也许使用 \cs{str_mdfive_hash:n} 会有帮助 ?
\end{todolist}
