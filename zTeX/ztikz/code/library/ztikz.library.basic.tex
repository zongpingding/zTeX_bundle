\ProvidesExplFile{ztikz.library.basic.tex}{2025/09/22}
  {1.0.0}{basic~library~for~ztikz}
...
\NewDocumentCommand\ShowGrid{ O{color=gray, very~ thin, step=1}m }
  {
    ...
    \draw[#1] \seq_item:Nn \l__grid_param_ii_seq{1} 
      grid \seq_item:Nn \l__grid_param_ii_seq{2};
  }
% intersection
...
\NewDocumentCommand\Polygon{ O{}m }
  {
    ...
    \int_step_inline:nnn {1}{#2}{
      ...
      \int_compare:nNnTF {##1}<{#2}
        {
          \tl_put_right:Nn \l__ztikz_basic_poly_path_tl 
            { (p##1)-- }
        }{
          \tl_put_right:Nn \l__ztikz_basic_poly_path_tl 
            { (p##1)--cycle }
    ...
    \fill [
        \tl_use:N \l__polygon_fill_color_tl, 
        fill~opacity=\fp_use:N \l__polygon_fill_opacity_fp
      ] \l__ztikz_basic_poly_path_tl;
    % show markers
    \int_step_inline:nnn {1}{#2}
      {
        \ShowPoint[\l__polygon_marker_option_tl]
          { (p##1) }
      }
    \end{scope} 
    \group_end:
...
\cs_new_protected:Npn \ztikz_draw_axis_ticks_cs:n #1 
  {
    \str_case:NnT \l__tick_type_tl {
      {main}{
        \dim_set_eq:NN \l__tick_length_dim
          \l__main_tick_length_dim
        \tl_set:NV \l__tick_color_tl
          \l__main_tick_color_tl
        \tl_set:Nn \l__node_text_tl 
          { \tl_use:N \l__main_tick_label_tl }
      }
      {sub}{
    ...
    \str_case:VnT \l__tick_spec_tl {
      ...
      {below}{
        \draw[\tl_use:N \l__tick_color_tl] (#1, 0) 
          -- ++(0, \dim_eval:n {-\l__tick_length_dim/2}) 
          node[\tl_use:N \l__main_tick_label_position_tl
            =\dim_use:N \l__tick_label_shift_dim] 
          {
            \textcolor{\tl_use:N \l__main_tick_label_color_tl}
              {\tl_use:N \l__node_text_tl}
          };
...
\NewDocumentCommand\ShowAxis{O{}m}
  {
    ...
    \draw[->, \tl_use:N \l__axis_color_tl] 
      \seq_item:Nn \l__points_seq{1} 
...
\NewDocumentCommand\StairsPlot
  { O{plot-left;jump-left}O{color=black}O{}m }
  {
    ...
    \str_case:enF {\seq_item:Nn \l__statistic_option_tl{1}}{
      ...
      \msg_new:nnn {ztikz}{ztikz-stairs-plot}
        {current~stairs~plot~type~is:~'#1'~,~ invalide}
    ...
    \str_case:enF {\seq_item:Nn \l__statistic_option_tl{2}}{
      ...
      \msg_new:nnn {ztikz}{ztikz-stairs-plot}
        {current~stairs~jump~type~is:~'#1'~,~ invalide}
      \msg_error:nn {ztikz}{ztikz-stairs-plot}
    }
    \ztikz_statistic_plot_cs:ennn {\l__tmpa_tl,\l__tmpb_tl}
      {#2}{#3}{#4}
  }
% stem plot
\NewDocumentCommand\StemPlot{ O{x}O{color=black}O{}m }
  {
    \str_case:enF {#1}{
      ...
      \msg_new:nnn {ztikz}{ztikz-stem-plot}
        {current~stem~plot~type~is:~'#1'~,~ invalide}
...
\NewDocumentCommand\BarPlot{ O{ybar}O{color=black}O{}m }
  {
    \str_case:enF {#1}{
      ...
      \msg_new:nnn {ztikz}{ztikz-bar-plot}
        {current~bar~plot~type~is:~'#1'~,~ invalide}
