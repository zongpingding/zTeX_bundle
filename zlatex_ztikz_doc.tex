...
% index make
% \makeindex[title=部分命令/名词索引, columns=3]
...
\section{兼容性}
目前本系列已经实现Windows和Linux下的兼容，但是MacOS下的兼容性还没有进行测试，所以不保证本系列可以在MacOS下正常运行.
具体的兼容情况请参见后续的兼容性章节.
...
\subsection{章节格式}
目前还不支持指定章节格式，等后续在添加. 使用者可以加载\cmd{titlesec}等宏包进行自定义.

\subsection{高亮环境}
目前本系列提供命令\cmd{\zlatexFramed{<name>}[<color>]}用于创建类似MarkDown的彩色引用环境. 参数中的
\cmd{<name>}表示声明环境的名称,\cmd{<color>}表示此环境的背景颜色.一个简单的使用样例如下:
...
证明类数学环境采用的是正体\cmd{\upshape}，如果使用者不喜欢前者默认的``斜体''字体样式，
...
都是``\cmd{;}''). \cmd{\ShowGrid}\index{\cmd{\ShowGrid}}命令的参数也是和\cmd{\ShowAxis}命令的参数一样的，只不过此命令中可以
指定一个\cmd{step}关键字，用于指定绘制网格的步长(间隔), 如\cmd{step=.5},设置步长为0.5. 对应的\cmd{\ShowPoint}\index{\cmd{\ShowPoint}}
命令的参数格式为:
...
zTikZ提供了命令\cmd{\pyfp}\index{\cmd{\pyfp}}用于浮点数运算, 这部分的结果并不会被缓存，也就是说每次编译此文档时，Python都会重新
