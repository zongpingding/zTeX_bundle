...
你就可以使用命令\cmd[F]{\enquote{<your text>}}.
...
中填诸如:\cmd{zihao=5, 12pt, heading=false, linespread=1.3} 等 {ctex} 手册中说明的合法选项. 若对于默认的 article,
book 文档类不熟悉，可以参见:\href{https://texblog.org/2013/02/13/latex-documentclass-options-illustrated/}{latex-documentclass-options}.

\subsection{fancy}
这一选项为 bool 值, 目前仅 \cmd[F]{\chapter} 命令和 \zkey{envStyle} 对此做了适配，后续可能会在此选项下开发更多的功能. 
...
\begin{itemize}
  \item \cmd[F]{\zsubtitle{<content>}}: 章节的副标题
  \item \cmd[F]{\zchapterSaying[<author>]{<saying>}}: 定义格言以及作者
  \item \cmd[F]{\zchapterLContent{<content>}}: 章节左侧的内容, 不允许含有\texttt{\textbackslash par}等命令
  \item \cmd[F]{\zchapterRContent{<content>}}: 章节右侧的内容, 不允许含有\texttt{\textbackslash par}等命令
\end{itemize}
...
\cref{fig:fancy-chapter-examples} 展示了部分使用样例:
\begin{figure}[!htb]
  \centering
...
\mstyle{slide Mode}\index{slide mode}
...
也就是使用符号``|''进行两个维度上距离的分割. 关于slide 模式下的主题颜色\index{slide theme}目前由以下三个名称控制，用户如果想要修改slide 的主题色，
请重定义它们:
...
\begin{tblr}{
  ...
  \cmd{zslideMain}: \block{zslideMain} &
  \cmd{zslideII}: \block{zslideII} &
  \cmd{zslideIII}: \block{zslideIII}
\end{tblr}
\end{table}

目前 slide 模式下提供了命令 \cmd[F]{\zslideMetadataSetup}\index{\cmd{\zslideMetadataSetup}}\index{slide theme} 用于设置 slide 的 
...
每一个 \texttt{<key>} 对应的的 content 请自行定义其前景色, 目前 \zkey{UL, UR} 对应的 content 默认位于右侧与左侧. 如果 
...
可以多次使用的命令\Footnote{众所周知，\cmd[F]{\maketitle} 会清除 \cmd[F]{\@title, \@author, \@date} 的定义}.

\mstyle{简单示例}
\cref{fig:slide-example} 为一默认 slide 模式样例:
\begin{figure}[!htb]
  \centering
  \includegraphics[width=.85\linewidth]{./pics/doc2slide_1.pdf}
  ...
  \includegraphics[width=.45\linewidth]{./pics/doc2slide_5.pdf}
  \includegraphics[width=.45\linewidth]{./pics/doc2slide_6.pdf}
...
\begin{table}[H]
  \begin{tblr}{
    ...
    Color & \block{RoyalRed} & \block{black} & \block{purple}& \block{RoyalRed} & \block{blue} & \block{zchapColor} & \block{zslideMain}\\
...
目前本模板还提供了命令\cmd[F]{\zlatexFramed{<name>}[<color>]}\index{\cmd{\zlatexFramed}}用于创建类似 MarkDown
...
\includepdf{./pics/ToC_1.pdf}
\includepdf{./pics/ToC_2.pdf}
...
\subsection{定理类环境样式}
z\LaTeX{}中的预想定义有6中样式\Footnote{若用户需要自定义样式，可参见后续\cmd{HOOK}选项}:\index{数学环境样式}\cmd{plain,leftbar,background,fancy,shadow,paris}. 其中\cmd{plain}表示数学环境
...
宏包提供了命令 \cmd[F]{\indexsetup{<options>}} 用于索引格式设定, 采用键值对的格式进行指定. 可用键:
...
文件:\cmd[F]{\jobname.ist}, 然后使用 makeindex 程序进行编译得到对应的 \cmd{.ind} 文件. 下面是一个示例 index 样式:
\begin{minted}{latex}
headings_flag 1
...
\includepdf{./pics/index-style-sample.pdf}

% \begin{figure}[!htb]
%   \centering
%   \includegraphics[width=.9\linewidth]{./pics/index-style-sample.pdf}
%   \caption{Index Style}
%   \label{fig:ist-file}
% \end{figure}
