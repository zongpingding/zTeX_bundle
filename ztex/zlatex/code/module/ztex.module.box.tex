...
% ==> better '\mbox' and '\fbox'(\verb can be used inside)
% REF: https://texhacks.blogspot.com/2011/01/better-mbox-and-fbox.html?m=1
% enhance \mbox
\def\zmbox{\leavevmode\hbox}

% universe box-content collect interface
\newbox\ztexcollectbox
\def\zboxcollectpre{}
\def\zboxcollect@pre
  {
    \bgroup
    \zboxcollectpre
    \aftergroup\zboxcollect@post
  }
\def\zboxcollectpost{}
\def\zboxcollect@post
  {
    \zboxcollectpost
    \egroup
    \zboxcollect@typeset
  }
\def\zboxcollect@typeset
  {
    \box\ztexcollectbox\relax
  }
\cs_set_protected:Npn \zbox_item_collect:nnnw #1#2#3
  {% #1:box typeset spec; #2/#3:box material pre/post
   % NOTE: box typeset is after 'post'.
    \cs_set:Npn \zboxcollect@typeset
      { #1 }
    \def\zboxcollectpre{#2}
    \def\zboxcollectpost{#3}
    \zboxcollect
  }

% users' interface
\protected\long\def\zboxcollect
  {
    \leavevmode
    \afterassignment\zboxcollect@pre
    \setbox\ztexcollectbox\hbox
  }
\NewDocumentCommand{\zboxcollectcustom}{mO{}O{}}
  { \zbox_item_collect:nnnw { #1 }{ #2 }{ #3 } }

% enhance \fbox
\ztex_keys_define:nn { box/enhance-fbox }
  {
    draw .tl_set:N = \l__zbox_fbox_draw_tl,
    draw .initial:n = { black },
    fill .tl_set:N = \l__zbox_fbox_fill_tl,
    fill .initial:n = { none },
    arc .fp_set:N = \l__zbox_fbox_arc_fp,
    arc .initial:n = { 1 },
    text .tl_set:N = \l__zbox_fbox_text_sty_tl,
    text .initial:n = {},
  }
\NewDocumentCommand\zfbox{o}
  {
    \IfValueT{#1}{\ztex_keys_set:nn { box/enhance-fbox } {#1}}
    \zbox_item_collect:nnnw
      {
        \begin{zpic} [
          unit=1pt, % \wd, \ht and \dp returns dim in 'pt', thus we must set 'unit=1pt'.
          xoffset=\dim_to_decimal:n { \fboxrule/2 },
          yoffset=-\dim_to_decimal:n
              { \dp\ztexcollectbox + \fboxsep + \fboxrule/2 },
          width=\dim_to_decimal:n
              { \wd\ztexcollectbox + 2\fboxsep + 2\fboxrule },
          height=\dim_to_decimal:n
              { \ht\ztexcollectbox + \fboxsep + \fboxrule },
          depth=\dim_to_decimal:n
              { \dp\ztexcollectbox + \fboxsep + \fboxrule },
        ]
        \zrectangle[
          arc=\l__zbox_fbox_arc_fp, width=\fboxrule,
          fill=\l__zbox_fbox_fill_tl, draw=\l__zbox_fbox_draw_tl
        ]
        ( 0, 0 )
        (
        \dim_to_decimal:n { \wd\ztexcollectbox + 2\fboxsep + \fboxrule },
        \dim_to_decimal:n
          {
            \ht\ztexcollectbox + \dp\ztexcollectbox + 2\fboxsep +
            \fboxrule
          }
        )
        \put
        (
        \dim_to_decimal:n
            { \wd\ztexcollectbox / 2 + \fboxsep + \fboxrule / 2 },
        \dim_to_decimal:n
            { \dp\ztexcollectbox + \fboxsep + \fboxrule / 2 }
        ) { \clap {\box_use_drop:N \ztexcollectbox} }
        \end{zpic}
        % reset fbox style options
        \ztex_keys_set:nn { box/enhance-fbox }
          {
...
\cs_new:Nn \__ztex_make_frame_options_restore:
  {
    \dim_set:Nn \l__zbox_frameduser_rulewd_dim   { 5pt }
    \tl_set:Nn  \l__zbox_frameduser_rulecolor_tl { red }
    \dim_set:Nn \l__zbox_frameduser_padding_dim  { 5pt }
    \tl_set:Nn  \l__zbox_frameduser_bgcolor_tl   { gray!10 }
    \dim_set:Nn \l__zbox_frameduser_boxadj_dim   { 0pt }
  }
...
\NewDocumentCommand\ztexframeend{}
  { 
    \ztex_make_frame_end: 
    \__ztex_make_frame_options_restore:
  }
