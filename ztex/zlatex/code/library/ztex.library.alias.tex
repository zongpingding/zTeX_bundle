\ProvidesExplFile{ztex.library.alias.tex}
  {2026/02/14}{\ztex@versi@n}
    ...
    \token_case_meaning:NnF \l_peek_token
      {
        ...
        {   (   }{ \__zab_case_match_auto: }
        {   [   }{ \__zab_case_match_auto: }
        {   <   }{ \__zab_case_match_auto: }
        {   |   }{ \__zab_case_match_auto: }
        {   \|  }{ \__zab_case_match_auto: }
        { \c_group_begin_token}{ \__zab_case_match_auto: }
      }{
...
\cs_new:Npn \__zab_case_match_auto: 
  {
...
\NewDocumentCommand\z@ab@curly {m}  {\zab@left \{#1 \zab@right\}}
\NewDocumentCommand\z@ab@round {r()}{\zab@left (#1  \zab@right)}
\NewDocumentCommand\z@ab@square{r[]}{\zab@left [#1  \zab@right]}
\NewDocumentCommand\z@ab@vert  {r||}{\zab@left |#1  \zab@right|}
\NewDocumentCommand\z@ab@Vert{r\|\|}{\zab@left \|#1 \zab@right\|} % double line
\NewDocumentCommand\z@ab@angle {r<>}{\zab@left \langle #1 \zab@right\rangle}
...
\cs_set_eq:NN \z@mat \zalias_matrix_from_list:n
\cs_set_eq:NN \z@mat@plain \zalias_matrix_from_list:n
\NewEnvironmentCopy{omatrix}{matrix}
% NOTE:
% 1. star      -> new ztex syntax
% 2. no star   -> traditional syntax
% 3. sometimes -> both syntaxes are compatible in star version
%                 if there is no open ',' or ';' inside
\cs_new_protected:Npn \__zalias_mat_typeset:n #1
  {
    \exp_args:Nc \NewDocumentCommand{z@#1mat}{sm}
      {
        \IfBooleanTF{##1}
          { \begin{#1matrix} \z@mat@plain{##2} \end{#1matrix} }
          { \begin{#1matrix} ##2 \end{#1matrix} }
      }
  }
\clist_map_inline:nn { o, p, b, B, v, V }
  { \__zalias_mat_typeset:n {#1} }
\ztex_mathalias_set:nn 
  { mat,   omat,   pmat,   bmat,   Bmat,   vmat,   Vmat   }
  { z@mat, z@omat, z@pmat, z@bmat, z@Bmat, z@vmat, z@Vmat }
...
\ztex_keys_define:nn { zalias/jhmat }
  {
    b   .tl_set:N   = \l__zalias_jhmat_border_tl,
    b   .initial:n  = { p },
    c   .tl_set:N   = \l__zalias_jhmat_cmd_tl,
    c   .initial:n  = { textstyle },
    s   .fp_set:N   = \l__zalias_jhmat_stretch_fp,
    s   .initial:n  = { 1.25 },
  }
\NewDocumentCommand{\z@jmat}{sO{}m} 
  {
    \IfBooleanTF{#1}
      {
        \group_begin:
          \ztex_keys_set:nn { zalias/jhmat }{ #2 }
          \renewcommand{\arraystretch}{\fp_use:N \l__zalias_jhmat_stretch_fp}
          \exp_args:No \begin{\l__zalias_jhmat_border_tl matrix}
            \exp_args:No \zalias_jmat_data:nn {\l__zalias_jhmat_cmd_tl}{#3} 
          \exp_args:No \end{\l__zalias_jhmat_border_tl matrix}
        \group_end:
      }{
        \exp_args:Ne \zalias_jmat_data:nn
          {\tl_if_empty:nTF {#2}{textstyle}{#2}}{#3} 
      }
  }
...
\NewDocumentCommand{\z@hmat}{sO{}m} 
  { 
    \IfBooleanTF{#1}
      {
        \group_begin:
          ...
          \renewcommand{\arraystretch}{\fp_use:N \l__zalias_jhmat_stretch_fp}
          \exp_args:No \begin{\l__zalias_jhmat_border_tl matrix}
            \exp_args:No \zalias_hmat_data:nn {\l__zalias_jhmat_cmd_tl}{#3} 
          \exp_args:No \end{\l__zalias_jhmat_border_tl matrix}
        \group_end:
      }{
        \exp_args:Ne \zalias_hmat_data:nn
          {\tl_if_empty:nTF {#2}{textstyle}{#2}}{#3} 
      }
  }
