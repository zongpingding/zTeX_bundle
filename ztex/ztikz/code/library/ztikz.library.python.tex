\ProvidesExplFile{ztikz.library.python.tex}{2025/05/21}{1.0.0}{python~library~for~ztikz}
...
% ==> users' interface
% python-matplotlib
\NewDocumentEnvironment{pyfig}{ m }
  {
    \xsim_file_write_start:ne {\c_true_bool}{\g__ztikz_python_path_tl/t@mp.py}
  }{
    \xsim_file_write_stop:   
    \ztikz_hash_if_change:neTF {\c_true_bool}{\g__ztikz_python_path_tl/t@mp.py}   
      {
        \ztool_append_to_file:nn {\g__ztikz_python_path_tl/t@mp.py} 
          { plt.savefig('#1') }
        ...
        \ztool_shell_mv:ee
          { #1 } 
          { \g__ztikz_python_path_tl/\l__ztikz_current_hash_tl _#1 }
        \ztikz_term_info:e {writing~source~to~file:'\g__ztikz_python_path_tl/pyfig_\l__ztikz_current_hash_tl.py'}
      }{
        \ztikz_term_info:e {skip~recompile~by~python,~using~the~cache~picture:'(\l__ztikz_current_hash_tl)_#1'}
      }
    \xdef\pyfigOutputFile{ \g__ztikz_python_path_tl/\l__ztikz_current_hash_tl _#1 }
  }
...
\NewDocumentEnvironment{pycode}{ m }
  {% #1:true to input result; #2:output file name (with ext)
    \xsim_file_write_start:ne {\c_true_bool}{\g__ztikz_python_path_tl/t@mp.py}
  }{ 
    \xsim_file_write_stop: 
    \ztikz_hash_if_change:neTF {\c_true_bool}{\g__ztikz_python_path_tl/t@mp.py}   
      {
        ...
        \ztool_shell_mv:ee 
          { #1 } 
          { \g__ztikz_python_path_tl/\l__ztikz_current_hash_tl _#1 } 
        \ztikz_term_info:e {writing~source~to~file:'\g__ztikz_python_path_tl/pycode_\l__ztikz_current_hash_tl.py'}
      }{
        \ztikz_term_info:e {skip~recompile~by~python,use~the~cache~result:'(\l__ztikz_current_hash_tl)_#1'}
      }
    \xdef\pycodeOutputFile{\g__ztikz_python_path_tl/\l__ztikz_current_hash_tl _#1}
  }