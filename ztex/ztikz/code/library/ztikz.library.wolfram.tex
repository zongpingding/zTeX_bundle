\ProvidesExplFile{ztikz.library.wolfram.tex}{2026/05/08}
  {1.2.0}{wolfram~library~for~ztikz}
...
\newcommand\wolframOutputPath{\g__ztikz_wolfram_path_tl}
...
\seq_new:N \g_ztikz_wolfram_res_seq
\ior_new:N \g__ztikz_wolfram_ior
\iow_new:N \g__ztikz_wolfram_iow


% ==> core function
\cs_new_protected:Npn \__ztikz_wolfram_execute:nnnn #1#2#3#4
  {% #1:contents(empty->not add); #2:extension; #3:output object; #4:label
    \__ztikz_wolfram_execute_prehook:nnn { #1 }{ #2 }{ #3 }
    \__ztikz_wolfram_execute_cli:nn      { #2 }{ #4 }
    \__ztikz_wolfram_execute_posthook:n  { #2 }
  }
\cs_generate_variant:Nn \__ztikz_wolfram_execute:nnnn { e, o }

% wolfram execute aux functions
\cs_new:Npn \__ztikz_wolfram_execute_prehook:nnn #1#2#3
  {
    \edef\@wolfram@tmp@file{\g__ztikz_wolfram_path_tl/t@mp}

    % add additional wolfram code - export process
    \tl_if_empty:nF {#1}{
      \__ztikz_addto_script:en { \@wolfram@tmp@file.wls }{ #1 }
    }

    % add additional wolfram code - script path
    \tl_if_empty:nT { #3 }
      {
        \exp_args:Neee \ztool_insert_to_file:nnn
          { \@wolfram@tmp@file.wls }{ 1 }
          { scriptPath~=~"\g__ztikz_wolfram_path_tl/" }
      }
    ...
    % add additional wolfram code - general export
    \tl_if_empty:nF { #3 }
      {
        \__ztikz_addto_script:ee { \wolfram@tmp@file.wls }
          { Export["\wolfram@tmp@file.#2", #3] }
        \xdef\wolframOutputFile  {
          \g__ztikz_wolfram_path_tl / \l__ztikz_current_hash_tl.#2
        }
      }
  }

\cs_new:Npn \__ztikz_wolfram_execute_posthook:n #1
  {
    \ior_open:Ne  \g__ztikz_wolfram_ior { \wolframOutputFile }
    \ior_get:NN   \g__ztikz_wolfram_ior \l__ztikz_wolfram_tmp_res_tl
    \tl_set_rescan:Nno \l__ztikz_wolfram_tmp_res_tl
      { \cctab_select:N \c_document_cctab }
      { \l__ztikz_wolfram_tmp_res_tl }
    \exp_args:NNe \seq_gset_split:NnV   \g_ztikz_wolfram_res_seq
      { \c_atsign_str } \l__ztikz_wolfram_tmp_res_tl
    \ior_close:N \g__ztikz_wolfram_ior
  }

\cs_new:Nn \__ztikz_wolfram_case_run:
  {
    \tl_if_eq:VnTF \g__ztikz_wolfram_engine_tl { wstp }
      {
        % NOTE: 'wstp' is much faster than the other engines
        \edef\wolfram@script
          {
            wstp~ '<<~ "\wolfram@tmp@file.wls"'
          }
      }
      {
        \edef\wolfram@script
    ...
    \ztool_shell_escape:e { \wolfram@script }
  }
\cs_new:Npn \__ztikz_wolfram_case_skip:n #1
  {% #1: ext
            ...
            \seq_if_in:NVF \g_ztikz_file_hash_seq \l__wolfram_current_hash_tl
              {
                ...
                \ztool_shell_rm:e {\wolfram@tmp@file.#1}
              }
          }
      }
    \ztikz_term_info:e
      {
        ...
        \l__ztikz_current_hash_tl.#1'\iow_newline:
      }
  }
\cs_new:Npn \__ztikz_wolfram_execute_cli:nn #1#2
  {% #1: ext; #2: hash label
    \ztikz_if_run_again:nenTF { \c_false_bool }{ \l__ztikz_current_hash_tl }{ #2 }
      {
        \__ztikz_wolfram_case_run:
      }{
        \__ztikz_wolfram_case_skip:n { #1 }
      }
  }
...
\msg_set:nnn {ztikz}{wolfram-arg_empty}
  { wolfram~library~error:~argument~of~command~'\c_backslash_str #1'~can~NOT~be~empty. }
\cs_new:Npn \__ztikz_wolfram_arg_empty_assert:nn #1#2
  {
    \exp_args:No \tl_if_empty:nT { #2 }
      {
        \msg_error:nnn {ztikz}{wolfram-arg_empty}{#1}
      }
  }
...
\NewDocumentCommand\wolframResult{so}
  {
    \IfBooleanTF{#1}
      {% must be integer expression, or it will raise bug.
        \seq_item:Ne \g_ztikz_wolfram_res_seq
          {\IfValueTF {#2}{\int_eval:n {#2}}{1}}
      }{
        \seq_use:Nn \g_ztikz_wolfram_res_seq
          { \IfValueTF {#2}{#2}{,} }
      }
  }

% --> wolfram code
\NewDocumentEnvironment{wolframCode}{m}
  {
    \xsim_file_write_start:ne {\c_false_bool}{\g__ztikz_wolfram_path_tl/t@mp.wls}
  }{
    \xsim_file_write_stop:
    \__ztikz_wolfram_execute:nnnn
      {}{}{}{#1}
  }
...
\NewDocumentEnvironment{wolframGraphics}{mO{}}
  {
    ...
    \xsim_file_write_stop:
    \__ztikz_wolfram_execute:nnnn {}{pdf}{FIGURE}{#1}
...
\NewDocumentCommand\wolfram{smm}
  {
    \__ztikz_wolfram_arg_empty_assert:nn {wolfram}{#3}
    \__ztikz_wolfram_execute:ennn
      {
        \IfBooleanTF{#1}
...
\group_begin:
  ...
  $gdef$wolframTex@getarg#1#2{
    $__ztikz_wolfram_arg_empty_assert:nn {wolframTex}{#2}
    $tl_set:Ne $l_tmpa_tl
      {
        $tl_map_function:nN {#2}
          $__double_backslash:n
      }
    $__ztikz_wolfram_execute:onnn
...
\cs_set:Npn \__table_item_handle:n #1
  {% the inner '\exp_not:N' prevent expansion from 'tabularray'.
    \exp_not:n {
...
\NewDocumentCommand{\wolframTable}{smO{}m}
  {% #1:if typeset; #2:key-value; #3:code
    \__ztikz_wolfram_arg_empty_assert:nn { wolframTable }{ #4 }
    \group_begin:
      \ztikz_keys_set:nn { wolfram/table }{ #3 }
      \__ztikz_wolfram_execute:ennn
        { TeXResult = #4;  }{ txt }
        { TeXResult }{ #2 }
      \__full_table_from_file:VV \wolframOutputFile
...
\NewDocumentCommand\wolframSolve{smom}
  { 
    \__ztikz_wolfram_arg_empty_assert:nn { wolframSolve }{ #4 }
    \group_begin:
...
\NewDocumentCommand\wolframDSolve{smom}
  {
    \__ztikz_wolfram_arg_empty_assert:nn { wolframDSolve }{ #4 }
    \group_begin:
