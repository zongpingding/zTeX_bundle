...
% operations precise
\fp_const:Nn \c__pic_precise_fp { 0.001 }

% command/env wrapper
\cs_new_protected:Npn \_@@_begin_picture:nnnn #1#2#3#4
  { \begin{picture}
      (\fp_eval:n {#1}, \fp_eval:n {#2})
      (\fp_eval:n {-#3}, \fp_eval:n {-#4}) }
\cs_new_protected:Npn \_@@_end_picture:
  { \end{picture} }
\cs_new_protected:Npn \__@@_pic_put:nnn #1#2#3
...
% width/height/depth are measured in units of `unit` and describe the
% dimensions reported to TeX.  In particular, `height` is the extent above
% the baseline and `depth` is the extent below it.  The picture environment
% itself always returns zero depth, so zpic collects its result and restores
% the requested box dimensions explicitly.
\box_new:N \l__pic_output_box
\cs_new:Npn \__pic_to_dim:n #1
  {
    \fp_to_dim:n
      { (#1) * \dim_to_fp:n { \l__pic_unit_dim } }
  }
\keys_define:nn { ztool / draw / picture }
  {
    ...
    height  .initial:n = 0,
    depth   .fp_set:N  = \l__pic_depth_fp,
    depth   .initial:n = 0,
...
\NewDocumentEnvironment{zpic}{O{}}
  {
    ...
    \_@@_end_picture:
    \hbox_set_end:
    \box_set_wd:Nn \l__pic_output_box
      { \__pic_to_dim:n { \l__pic_width_fp  } }
    \box_set_ht:Nn \l__pic_output_box
      { \__pic_to_dim:n { \l__pic_height_fp } }
    \box_set_dp:Nn \l__pic_output_box
      { \__pic_to_dim:n { \l__pic_depth_fp  } }
    \box_use_drop:N \l__pic_output_box
    \group_end:
  }
...
\cs_new_protected:Npn \ztool_pic_line_vector:nnnn #1#2#3#4
  {% #1:line/vector; #2:key-value; #3:start coor; #4:end coor;
    ...
    \fp_compare:nNnTF { \__coor_st:n {#4} - \__coor_st:n {#3} } > { \c__pic_precise_fp }
      {
...
% --> arc rule: similar to arc by it's a rule
\cs_new:Npn \__@@_pic_arcrule:nnn #1#2#3
  {
    % To be implemented ... 
  }
  ...
  {% #1:key-value; #2:coor
    ...
    \exp_last_unbraced:Ne \__@@_pic_put:nnn 
      { \__coor_st_nd:n {#2} }
      {
        ...
        % NOTE: border must draw after the fill
        \__pic_set_line_color:
        \exp_args:Ne \__@@_pic_arc:nnnn {}
          { \fp_use:N \l__pic_arc_start_fp  }
          { \fp_use:N \l__pic_arc_end_fp    }
...
\clist_const:Nn \l__pic_rec_quadrant_shift_prop
    { {(0, 0)}, {(-\rec@arc, 0)}, {(-\rec@arc, -\rec@arc)}, {(0, -\rec@arc)} }
\cs_new_protected:Npn \ztool_pic_rectangle:nnn #1#2#3
  {% #1:key-value; #2:start coor; #3:end coor;
    ...
    % check if arc radius too large.
    \edef\rec@arc
      {
        \fp_eval:n { min(min(\rec@height/2, \rec@width/2), \l__pic_rec_arc_fp) }
      }
    \__region_fill_color_miss:n { gray }
    \__color_if_valid:VF \l__pic_region_fill_color_tl
      { 
    ...
    \__pic_rectangle_rec_comp_fill:n { #2 }
    \__pic_rectangle_arc_comp_fill:n { #2 }
...
% aux function for rectangle area filling in rectangle
\cs_new:Npn \__pic_rectangle_rec_comp_fill:n #1
  {
    \__@@_pic_put:nnn {\__coor_st:n {#1}+\rec@arc}{\__coor_nd:n {#1}}
      {
        \__pic_set_fill_color:
        \rule
          {\fp_eval:n {(\rec@width-2*\rec@arc)*\dim_to_decimal:n {\l__pic_unit_dim}}pt}
          {\fp_eval:n {\rec@height*\dim_to_decimal:n {\l__pic_unit_dim}}pt}
      }
    \fp_compare:nNnT { \rec@height-2*\rec@arc } > { \c__pic_precise_fp }
      {
        \__@@_pic_put:nnn {\__coor_st:n {#1}}{\__coor_nd:n {#1}+\rec@arc}
          {
            \__pic_set_fill_color:
            \rule
               {\fp_eval:n {\rec@arc*\dim_to_decimal:n {\l__pic_unit_dim}}pt}
               {\fp_eval:n {(\rec@height-2*\rec@arc)*\dim_to_decimal:n {\l__pic_unit_dim}}pt}
          }
        \__@@_pic_put:nnn {\__coor_st:n {#1}+\rec@width-\rec@arc}{\__coor_nd:n {#1}+\rec@arc}
          {
            \__pic_set_fill_color:
            \rule
              {\fp_eval:n {\rec@arc*\dim_to_decimal:n {\l__pic_unit_dim}}pt}
...
% aux function for arc area filling in rectangle
\cs_new:Npn \__pic_rectangle_arc_comp_fill:n #1
  {
    \int_set:Nn \l__pic_rec_quadrant_index_int { 0 }
    \tl_map_inline:nn 
      { 
        {\__coor_st:n {#1}+\rec@width-\rec@arc, \__coor_nd:n {#1}+\rec@height-\rec@arc} 
        {\__coor_st:n {#1}+\rec@arc,            \__coor_nd:n {#1}+\rec@height-\rec@arc} 
        {\__coor_st:n {#1}+\rec@arc,            \__coor_nd:n {#1}+\rec@arc} 
        {\__coor_st:n {#1}+\rec@width-\rec@arc, \__coor_nd:n {#1}+\rec@arc} 
      }{
