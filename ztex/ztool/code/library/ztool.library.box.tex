\ProvidesExplFile{ztool.library.box.tex}
  {2026/02/13}{1.0.1}
  {box~library~for~ztool}
...
\cs_generate_variant:Nn \ztool_get_ht:Nn  
  { Ne, ce }
\cs_generate_variant:Nn \ztool_get_ht_plus_dp:Nn  
  { Ne, ce }
\cs_generate_variant:Nn \ztool_get_wd:Nn  
  { Ne, ce }
\cs_generate_variant:Nn \ztool_gget_ht:Nn 
  { Ne, ce }
\cs_generate_variant:Nn \ztool_gget_wd:Nn 
  { Ne, ce }
...
\cs_new_protected:Npn \ztool_autoset_to_wd_and_ht:nnn #1#2#3
  {% #1:width; #2:height; #3:object 
    \hbox_set:Nn \l_ztool_tmpa_box {#3}
    \box_autosize_to_wd_and_ht:Nnn \l_ztool_tmpa_box {#1}{#2}
    \__ztool_leave_vmode:
    \box_use:N \l_ztool_tmpa_box
  }
\cs_new_protected:Npn \ztool_rotate:nn #1#2
  {% #1:angle; #2:object
    \hbox_set:Nn \l_ztool_tmpa_box {#2}
    \box_rotate:Nn \l_ztool_tmpa_box {#1}
    \__ztool_leave_vmode:
    \box_use:N \l_ztool_tmpa_box
  }
\cs_generate_variant:Nn \ztool_rotate:nn 
  { e, ne, ee }
\cs_generate_variant:Nn \ztool_autoset_to_wd_and_ht:nnn 
  { nne, een, eee }
...
\cs_new_protected:Npn \ztool_set_to_wd:nn #1#2
  {% #1:width; #2:object
    \hbox_set:Nn \l_ztool_tmpa_box {#2}
    \box_resize_to_wd:Nn \l_ztool_tmpa_box {#1}
    \__ztool_leave_vmode:
    \box_use:N \l_ztool_tmpa_box
  }
\cs_new_protected:Npn \ztool_set_to_ht:nn #1#2
  {% #1:height; #2:object
    \hbox_set:Nn \l_ztool_tmpa_box {#2}
    \box_resize_to_ht:Nn \l_ztool_tmpa_box {#1}
    \__ztool_leave_vmode:
    \box_use:N \l_ztool_tmpa_box
  }
...
% NOTE: if boxwd{content} <= given dim, no manipulation
\cs_new_protected:Npn \ztool_scale_to_wd:nn #1#2
  {
    \hbox_set:Nn \l_ztool_tmpa_box {#2}
    \dim_set:Nn \l_tmpa_dim { \box_wd:N \l_ztool_tmpa_box }
    \fp_set:Nn \l_tmpa_fp  
      { 
        \fp_eval:n { min(1, \dim_ratio:nn {#1}{\l_tmpa_dim}) }
      }
    \box_scale:Nnn \l_ztool_tmpa_box {\l_tmpa_fp}{1}
    \__ztool_leave_vmode:
    \box_use:N \l_ztool_tmpa_box
  }
\cs_new_protected:Npn \ztool_scale_to_ht:nn #1#2
  {% take depth into consideration
    \hbox_set:Nn \l_ztool_tmpa_box {#2}
    \dim_set:Nn \l_tmpa_dim { \box_ht_plus_dp:N \l_ztool_tmpa_box }
    \fp_set:Nn \l_tmpa_fp  
      { 
        \fp_eval:n { min(1, \dim_ratio:nn {#1}{\l_tmpa_dim}) }
      }
    \box_scale:Nnn \l_ztool_tmpa_box {1}{\l_tmpa_fp}
    \__ztool_leave_vmode:
    \box_use:N \l_ztool_tmpa_box
  }
\cs_new_protected:Npn \ztool_scale_to_wd_and_ht:nnn #1#2#3
  {% take depth into consideration
    \hbox_set:Nn \l_ztool_tmpa_box {#3}
    \dim_set:Nn \l_tmpa_dim { \box_wd:N \l_ztool_tmpa_box }
    \dim_set:Nn \l_tmpb_dim { \box_ht_plus_dp:N \l_ztool_tmpa_box }
    \fp_set:Nn \l_tmpa_fp  
      { 
        \fp_eval:n { min(1, \dim_ratio:nn {#1}{\l_tmpa_dim}) }
      }
    ...
    \box_scale:Nnn \l_ztool_tmpa_box {\l_tmpa_fp}{\l_tmpb_fp}
    \__ztool_leave_vmode:
    \box_use:N \l_ztool_tmpa_box
  }
\cs_generate_variant:Nn \ztool_scale_to_wd:nn 
  { e, ne, ee }
\cs_generate_variant:Nn \ztool_scale_to_ht:nn 
  { e, ne, ee }
\cs_generate_variant:Nn \ztool_scale_to_wd_and_ht:nnn 
  { nne, nno, eee }
...
\cs_set_protected:Npn \ztool_box_item_align:Nnnn #1#2#3#4
  {% #1:cmd, #2:width, #3:object, #4:align format(left, right, scatter, center)
    \hb@xt@#2{
      \tl_map_inline:nn {#3} 
        {
          \seq_put_right:No \l__ztool_boxitem_seq 
            { \exp_not:N #1{##1} }
        } 
      \str_case:nnF { #4 }
        {
...
\cs_generate_variant:Nn \ztool_box_item_align:Nnnn 
  { c, Nnno, cnno, Nne, Nnee }
...
\cs_new:Npn \ztool_affine_transformation:Nnnnn #1#2#3#4#5
  {% #1:box; #2:$a_{11}$; #3:$a_{21}$; #4:$a_{12}$; #5:$a_{22}$.
    ...
    \coffin_typeset:Nxxxx \l__affine_trans_coffin 
      { \l__ztool_affine_pole_a_tl }
      { \l__ztool_affine_pole_b_tl }
      { \l__ztool_affine_xoffset_dim }
      { \l__ztool_affine_yoffset_dim }
  }
\cs_generate_variant:Nn \ztool_affine_transformation:Nnnnn 
  { Neeee, cnnnn, ceeee }
